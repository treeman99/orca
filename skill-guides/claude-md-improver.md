...
Orca 사내 배포판이 번들한 서드파티 스킬이다. **본문은 원문 그대로이고 이 절만 추가했다.**

- 출처: Anthropic `claude-md-management` 플러그인 v1.0.0 · `skills/claude-md-improver/`
- 함께 설치되는 참조 문서: `references/quality-criteria.md`, `references/templates.md`,
  `references/update-guidelines.md`
- 라이선스: Apache-2.0 (이 디렉터리의 `LICENSE`)
- 네트워크: 이 스킬은 URL을 조회하지 않는다. 본문의 `npm install`은 CLAUDE.md에 써 넣을
  Quick Start 예시 블록 안의 문자열이지 실행 지시가 아니다.
