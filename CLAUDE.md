...
- **Skill guides** — direction is the reverse of what it looks like: `skill-guides/<topic>.md` is the authoritative source and `skills/<topic>/SKILL.md` is **generated** from it. A topic listed in `STUB_TOPICS` gets a thin discovery stub whose body comes from `skill-stubs/<topic>.md`; every other topic gets the full guide byte-for-byte. Hand-written siblings under `skills/<topic>/` (reference docs, `LICENSE`) are left alone by the generator and ship with the package. After editing a skill run `pnpm generate:bundled-skill-guides` and `pnpm generate:skill-bundle-manifest`. Guide names in `config/scripts/generate-bundled-skill-guides.mjs` are a compatibility ledger — renames add aliases, never remove them.
