...
| **더블클릭** | 우측 본 화면을 그 봇의 **에이전트 세션**으로 전환합니다(그 봇에 지정한 에이전트 — claude, codex 등). 세션이 없으면 띄우고 나서 엽니다. `Chat UI`를 켜 두었다면 **말풍선 채팅 뷰**로 열립니다(§8.4) |
...
### 8.4 채팅 UI로 보기 (Chat UI)

봇 대화는 실제로는 **터미널 탭**에서 돕니다. 그 탭을 **채팅 뷰**로 바꾸면 에이전트의 전사를
읽어 **말풍선 스레드**로 그려 줍니다. 터미널 화면을 읽을 필요가 없어집니다.

#### 켜는 법 — 한 번만

> **설정 → Experimental → `Chat UI`** 토글 ON

⚠️ **이 토글이 꺼져 있으면 아무 일도 일어나지 않습니다.** 봇을 열어도 터미널 화면 그대로입니다.
실험 기능이라 기본값이 꺼짐입니다.
...
| 방법 | 어디서 |
| --- | --- |
| 로스터에서 봇을 **더블클릭** | `Bots` 탭 목록 |
| 봇 상세의 **↗** 버튼 | 봇을 연 뒤 헤더 오른쪽 |

둘 다 그 봇의 세션을 본 화면에 띄우면서 **채팅 뷰로 전환**합니다. 되돌리려면 탭에서 다시
터미널 뷰로 토글하면 됩니다 — 같은 세션이고, 보는 방식만 바뀝니다.
...
#### ⚠️ 원격(SSH) 워크스페이스의 봇은 안 됩니다

채팅 뷰는 **이 PC의 디스크에 있는 전사 파일**을 읽습니다. SSH 워크스페이스에서 도는 봇의
전사는 저쪽 호스트에 있어서 읽을 수 없고, 그래서 채팅 뷰가 열리지 않습니다. 그 봇은 터미널
화면으로 보셔야 합니다.
