...
> 앱 안에도 같은 설명서가 있습니다 — 사이드바 맨 아래 **`?` → 봇 사용법**. 그림이 함께 있어
> 화면을 보면서 읽기에는 그쪽이 낫습니다. 이 절은 그 내용에 관리자용 항목을 더한 판입니다.

좌측 사이드바 위쪽의 **`Sessions | Bots`** 탭에서 씁니다. `Sessions`는 지금까지 쓰던 워크스페이스
목록 그대로이고, `Bots`가 새로 붙은 레인입니다. 봇 목록은 **프로젝트별로 묶여** 보입니다 —
위임도 같은 프로젝트 안에서만 일어나므로 그 경계를 그대로 드러냅니다.

프로젝트 이름을 누르면 그 그룹이 **접히고 펼쳐집니다**. 접힌 그룹에 읽지 않은 메시지가 있으면
점으로, 안에서 뭔가 돌고 있으면 아이콘으로 표시됩니다.

로스터 행의 아이콘:

| 표시 | 뜻 |
| --- | --- |
| 스피너 | **작업 중** — 그 봇의 에이전트가 턴을 돌고 있습니다 |
| ⏳ | **응답 대기** — 자기 턴은 끝났고, 자기가 넘긴 팀메이트가 아직 돌고 있습니다 |
| (없음) | 대기 상태이거나 세션이 아직 없습니다 |

이름과 역할은 패널 폭에 맞춰 **줄바꿈**됩니다(잘리지 않습니다).

봇 목록 위에는 **`Rooms`** 섹션이 있습니다 — 여러 봇이 함께 있는 그룹 방입니다(§8.7).
방이 도는 중이면 그 행에도 스피너가 붙습니다.

| 로스터에서 | 동작 |
| --- | --- |
| 클릭 · 행 오른쪽 아이콘 | **봇 화면** — 루틴 설정과 메시지 입력창이 있는 그 화면 |
| **더블클릭** | 우측 본 화면을 그 봇의 **에이전트 세션(터미널)**으로 전환합니다(그 봇에 지정한 에이전트 — claude, codex 등). 세션이 없으면 띄우고 나서 엽니다. 말풍선 채팅은 **그룹 방**입니다 — §8.4 |

봇의 이름·아바타·에이전트·프로젝트 **편집**은 봇 화면 헤더의 ✏️ 입니다.

봇에게는 세 가지 방식으로 일을 시킵니다 — **채팅으로 직접**(§8.3), **여러 봇을 한 방에 모아**(§8.7),
또는 **예약 루틴으로**(§8.8). 대화를 터미널이 아니라 말풍선으로 보고 싶다면 §8.4를 먼저 보세요.
...
2. **아바타 / 이름 / 역할 / 설명**을 채웁니다
   - ⚠️ **역할과 설명은 표시용이 아니라 에이전트에게 전달되는 지시문입니다.**
     설명에 "이 저장소에서만 찾아라", "이 URL을 먼저 읽어라" 처럼 **범위**를 적으면
     대화든 예약 루틴이든 그 지시를 지고 실행됩니다(§8.10).
...
4. **프로젝트**를 고릅니다. 봇은 그 프로젝트에서 일합니다.
   - 실제로 어느 체크아웃에서 도는지는 Orca가 정합니다(메인 워크트리 우선). 기능 워크트리는
     브랜치가 병합되면 사라져서, 거기 묶인 봇이 통째로 고아가 되기 때문입니다.
   - 나중에 정해도 됩니다. 다만 프로젝트가 없으면 대화도 루틴도 만들 수 없습니다.
...
### 8.4 어디가 터미널이고 어디가 채팅인가

봇을 쓸 때 화면이 두 종류 나옵니다. **둘은 역할이 다르고, 헷갈리면 오류를 놓칩니다.**

| | 무엇 | 어떻게 여는가 | 왜 이 형태인가 |
| --- | --- | --- | --- |
| **봇 세션** | 그 봇 **하나**의 에이전트가 도는 **터미널** | 로스터에서 봇 더블클릭, 봇 상세의 **↗**, 탭바 `+` → `봇 대화` | 에이전트 원본 출력이 그대로 보입니다. **실패·오류를 확인하는 자리** |
| **그룹 방(Room)** | 여러 봇이 함께 있는 **말풍선 채팅** | `Bots` 탭 → `Rooms` → `+` | 여러 세션의 답변을 **하나의 기록**으로 모읍니다(§8.7) |

즉 **채팅은 방 하나로 관리하고, 각 봇의 세션은 터미널로 따로 돕니다.** 방이 멤버들에게 지시를
보내고 각자의 세션에서 답변을 읽어 와 한 줄씩 쌓습니다 — 방은 세션을 **대체하지 않고 읽습니다.**

> ⚠️ 예전 버전은 봇을 더블클릭하면 그 봇 **전용 채팅 창**이 떴습니다. 봇이 3개면 채팅도 3개라
> 서로 뭘 하는지 안 보이고, 위임이 실패하면 그 봇이 자기 창에서 조용해지는 것으로만 나타났습니다.
> 그래서 봇 개별 화면은 **터미널로 되돌렸습니다.** 필요하면 그 탭에서 직접 채팅 뷰로 토글할 수
> 있습니다 — 같은 세션이고 보는 방식만 바뀝니다.

#### 개별 탭을 채팅 뷰로 토글하고 싶다면

> **설정 → Experimental → `Chat UI`** 가 켜져 있어야 합니다 (이 포크는 **기본 켜짐**).

토글은 탭에서 직접 합니다. 되는 에이전트가 정해져 있습니다:

| 에이전트 | 채팅 뷰 |
| --- | --- |
| `claude`, `openclaude`, `codex`, `grok`, `omp`, `opencode` | ✅ |
| 그 외 (`gemini` 등) | ❌ 전사 형식을 읽을 수 없어 토글 자체가 걸리지 않습니다 |

⚠️ **원격(SSH) 워크스페이스의 봇은 안 됩니다.** 채팅 뷰는 **이 PC 디스크의 전사 파일**을 읽습니다.
SSH 워크스페이스에서 도는 봇의 전사는 저쪽 호스트에 있어 읽을 수 없습니다(`grok`·`omp`·`opencode`는
로컬·WSL 전용). 그 봇은 터미널 화면으로 보셔야 합니다.

### 8.5 채팅 뷰가 어떻게 보이는가 (토글했을 때 / 방에서)

**중요한 성질 하나: 이건 Orca가 따로 저장한 사본이 아닙니다.** 에이전트 CLI가 디스크에 쓴
전사를 그대로 읽어 그립니다. 그래서 화면에 보이는 것이 곧 그 에이전트가 실제로 주고받은
내용이고, 둘이 어긋날 수 없습니다.

- **말풍선 스레드** — 내가 보낸 프롬프트와 봇의 답변이 턴 단위로 쌓입니다.
- **도구 사용도 보입니다** — 파일을 읽고 명령을 돌린 내역이 함께 그려집니다. 터미널의
  ANSI 화면이 아니라 구조화된 항목이라 훑기 쉽습니다.
- **길이 제한이 없습니다** — 사이드바 미리보기는 8000자에서 잘리지만, 채팅 뷰는 전사를
  직접 읽으므로 답변 전문이 그대로 나옵니다.
- **입력창이 있습니다** — 채팅 뷰에서 바로 다음 지시를 보낼 수 있습니다. 같은 세션으로
  들어가므로 사이드바에서 보내는 것과 결과가 같습니다.
- **살아 있는 화면입니다** — 봇이 답하는 동안 새 내용이 따라 붙습니다.

#### Confluence 연결이 401/301로 실패할 때

봇이 사내 위키를 읽게 하려면 **설정 → 연동 → 사내 Confluence**에 URL과 토큰을 넣고
**연결 테스트**를 누릅니다. Orca는 `Authorization: Bearer <토큰>`으로 호출합니다 — 사내 안내와
같은 형식입니다.

| 결과 | 뜻과 대처 |
| --- | --- |
| **리다이렉트된다고 나온다** | 넣은 URL이 최종 주소가 아닙니다. 메시지가 **가야 할 주소를 알려 주니** 그걸 URL 칸에 넣으세요. `http://`로 넣으면 서버가 `https://`로 301을 주는 경우가 가장 흔합니다 |
| **토큰이 거부됐다(401/403)** | 주소는 맞고 토큰이 문제입니다. 토큰을 다시 발급하세요 |
...
> 💡 **URL은 위키 페이지 주소를 그대로 붙여넣어도 됩니다.** `…/display/TEAM/문서`, `…/pages/viewpage.action?pageId=…`
> 같은 주소를 넣으면 앞부분(기지 주소)만 남기고 잘라 냅니다. 예전에는 그 경로가 그대로 남아
> `…/display/TEAM/문서/rest/api/space` 를 부르는 바람에 **멀쩡한 토큰으로도 401**이 났습니다.

> ⚠️ **왜 리다이렉트를 따라가지 않는가** — 웹 표준상 리다이렉트가 **주소(origin)를 바꾸면
> `Authorization` 헤더가 제거**됩니다(`http:`→`https:`도 바뀐 것으로 봅니다). 그대로 따라가면
> 토큰 없이 재요청이 나가고, 서버는 당연히 **401**을 돌려줍니다. 즉 *"301도 나고 401도 난다"*는
> 토큰 문제가 아니라 **주소 문제**입니다. 그래서 Orca는 따라가지 않고 갈 주소를 알려 줍니다.

#### 사이드바 스레드와의 관계

봇 상세의 대화 스레드는 **여전히 인덱스**입니다 — 내가 보낸 것, 다른 봇이 넘긴 것, 그리고
가장 최근 답변 하나. 전체 대화는 채팅 뷰(또는 터미널)에 있습니다. 사이드바가 요약이고
채팅 뷰가 원본이라고 보시면 됩니다.
...
**팀메이트는 Orca가 미리 띄웁니다.** 봇에게 일을 시키면, 그 전에 **같은 프로젝트의 다른 봇들**
중 아직 안 뜬 것들을 대기 상태로 올립니다. 그래야 코디네이터가 팀 목록을 봤을 때 실제로 거기
있습니다. 한 번에 최대 6개까지입니다 — 하나하나가 실제 에이전트 프로세스라 쿼터를 씁니다.

⚠️ 그래도 못 뜬 팀메이트가 있으면, 프리앰블이 `orca terminal create --title "bot:<핸들>"`로
**직접 띄우는 방법**을 알려 줍니다. 제목이 맞으면 Orca가 그 터미널을 그 봇의 대화로 채택합니다.
...
주소 체계는 **터미널 제목 하나**가 전부입니다. 모든 봇 대화는 `bot:<핸들>` 이라는 제목의
터미널에서 돌고, 봇은 그 제목으로 서로를 찾습니다.

| 규칙 | 내용 |
| --- | --- |
| 누가 팀메이트인가 | **같은 프로젝트**에 속하고 **워크스페이스가 있는** 봇. 프로젝트가 없는 봇은 명부에 오르지 않습니다 |
| 봇이 **스스로** 넘길 수 있는 범위 | **프로젝트를 넘지 않습니다.** 봇에게 주입되는 팀메이트 명부에는 같은 프로젝트 봇만 오릅니다 |
| 내가 `@핸들`로 **직접** 부를 때 | 로스터 **전체**가 대상입니다 — 다른 프로젝트 봇도 갑니다(그 봇에 워크스페이스가 있으면). 프로젝트 경계는 봇의 자율 위임에만 걸립니다 |
| 핸들 생성 | 이름을 소문자로, 공백은 하이픈으로, 한글은 그대로. `릴리스 점검` → `@릴리스-점검`. 이름을 바꾸면 핸들도 따라갑니다 |
| 찾는 방법 | `orca terminal list --json` 으로 `bot:` 제목을 훑고, `orca terminal send` 로 씁니다 |
| 없으면 | `orca terminal create --title "bot:<핸들>" --background` 로 직접 띄웁니다. 제목이 맞으면 Orca가 그 터미널을 그 봇의 대화로 채택합니다 |
| 팀메이트가 없으면 | 위임 이야기를 **아예 하지 않습니다** — 그냥 자기가 합니다. 혼자인 봇에게 없는 동료를 찾게 만들지 않기 위해서입니다 |

#### 다른 봇을 명시적으로 지정할 때 — 프롬프트 쓰는 법

이게 가장 많이 어긋나는 지점입니다. **받는 봇은 당신의 대화를 보지 못합니다.**
멘션 뒤의 문장과, 보낸 봇이 누구인지만 봅니다. 그러니 **그것만 읽고도 일이 되도록** 씁니다 —
무엇을, 어디서, 무엇으로 돌려줄지.
...
| 쓴 것 | 왜 안 되나 |
| --- | --- |
| `@analyzer 아까 그거 분석해줘` | 상대 봇에게는 "아까"가 없습니다. 맥락이 전달되지 않습니다 |
| `이건 @analyzer 한테 물어봐` | **맨 앞의 멘션만** 전달됩니다. 이건 지금 대화 중인 봇에게 그대로 남습니다 |
| `@anaylzer …` (오타) | 없는 핸들은 **보내지 않고 알려 줍니다.** 오타가 대화 속으로 조용히 사라지지는 않습니다 |
| 워크스페이스 없는 봇에게 | 프로젝트가 비어 있으면 세션을 띄울 수 없어 전달이 실패합니다 |

코디네이터 봇이 **직접 하지 않고 매번 위임하게** 하려면, 그 규칙을 그 봇의 **설명**에 적으십시오
(설명은 상시 지시로 전달됩니다 — §8.10):

```
분석은 반드시 @analyzer 에게, 리포트는 @리포터 에게 넘겨라. 직접 하지 마라.
넘길 때는 상대가 내 대화를 못 본다고 가정하고, 대상 경로와 원하는 산출물을 문장에 다 적어라.
```

### 8.7 그룹 방 — 여러 봇이 한 방에서

`@핸들`이 **한 명에게 넘기는 것**이라면, 그룹 방은 **여러 봇이 같은 대화를 보고 순서대로
발언하는 것**입니다. 방 하나가 하나의 이어진 기록을 갖습니다.

#### 만들기

> `Bots` 탭 상단 **`Rooms`** 섹션의 **+** 버튼

1. 같은 프로젝트의 봇 **2~6명**을 고릅니다.
2. 방 이름을 적습니다. **비워 두면 고른 멤버 이름이 그대로 방 이름**이 됩니다.
3. 만들면 바로 방이 열립니다.

**+ 버튼이 비활성이면** 전제가 안 맞은 것입니다:

| 필요한 것 | 이유 |
| --- | --- |
| 봇 **2명 이상** | 방은 최소 두 명입니다 |
| **같은 프로젝트** | 방은 프로젝트를 넘지 않습니다 — 다른 체크아웃의 봇은 여기서 띄울 수 없습니다 |
| **워크트리**에 바인딩 | 폴더 워크스페이스는 거부됩니다(§8.10) |

#### 메시지를 보내면 무슨 일이 일어나는가

```
나: 결제 버그 좀 봐줘 @radar
    ↓
Radar가 답한다 → "마이그레이션 쪽은 @builder가 아는 것 같은데"
    ↓  (봇이 부른 봇은 다음 라운드에 합류합니다)
Builder가 답한다
    ↓
아무도 더할 말이 없다 → 전원 (pass) → 대화 정착, 끝
```

- **누가 답하는가**: 문장 **어디든** `@핸들`이 있으면 그 봇들만. `@everyone` / `@all`이면 전원.
  아무도 안 부르면 전원. 매 라운드 다시 계산하므로, 봇이 부른 봇이 다음 라운드에 들어옵니다.
  (1:1 대화의 `@핸들`은 **맨 앞에 있을 때만** 넘어가지만, 방에서는 어디에 있든 주소입니다.)
- **한 번에 한 명씩** 순서대로 돕니다. 동시에 답하지 않습니다.
- **라운드마다 시작 발언자가 바뀝니다.** 안 그러면 늘 같은 봇이 먼저 말해 판을 잡고 나머지는
  "이미 나온 얘기"라며 침묵합니다.
- **할 말이 없으면 `(pass)`** 로 답하게 되어 있습니다. 침묵은 좋은 것이고, 전원이 pass한
  라운드가 나오면 그 대화는 **정착된 것으로 보고 끝냅니다.**
- 방이 도는 중에 새로 보내면, 돌던 것은 다음 발언 경계에서 멈추고 새 메시지가 넘겨받습니다.
...
| 한 번 보낼 때 봇 발언 총합 | **10** |
| 방 멤버 | **6** |
| 프롬프트에 싣는 최근 방 기록 | **24줄** |

봇끼리 서로를 계속 부르면 끝나지 않을 수 있어서, 상한은 협상 대상이 아니라 코드에 박혀
있습니다. 상한에 닿으면 그냥 멈춥니다.

#### 진행 표시 — 멈춘 것과 일하는 것을 구분합니다

방이 도는 동안 기록 맨 아래에 한 줄이 붙습니다:

```
🤖  ⟳  Edit · src/renderer/app.ts              1:24   라운드 2/3 · 3/10 발언
│   │   └ 그 봇이 지금 쓰는 도구                 │      └ 남은 작업량
│   └ 회전 아이콘                                └ 경과 시계 (1초마다)
└ 봇 얼굴이 작업 자세로 움직입니다
```

모델 턴은 몇 분씩 아무 줄도 안 뱉을 수 있어서, **시간이 가고 있다는 것 자체**가 살아 있다는
증거입니다.

⚠️ **45초 동안 아무 소식이 없으면 애니메이션이 멈추고 `아직 소식 없음`으로 바뀝니다.**
멈춘 것을 계속 도는 것처럼 보여 주지 않으려는 것입니다. 이 표시가 나와도 아직 포기한 것은
아니고, 그 턴은 최대 3분(작업이 확인되면 최대 20분)까지 기다립니다.

#### 스레드

메인 입력창은 **새 스레드**를 엽니다. 스레드 안의 답장 상자는 **그 스레드를 잇습니다.**
가장 최근 스레드만 펼쳐져 있고 나머지는 한 줄로 접힙니다. Slack의 채널과 스레드에 가깝습니다.

#### ⚠️ 방은 프로세스를 더 씁니다

멤버는 방마다 **자기 방 전용 세션**을 따로 갖습니다(터미널 제목 `bot:<핸들>@<방id>`).
1:1 대화와 섞이지 않게 하려는 것입니다 — 방 규칙에 "1:1 대화 내용을 방에 흘리지 말 것"이
들어 있는데, 같은 세션을 쓰면 그 약속이 성립하지 않습니다.

**즉 4명짜리 방을 돌리면 에이전트 프로세스가 4개 더 뜹니다.** 전부 실제 쿼터를 씁니다.

#### 방 기록은 저장됩니다 — 다만 "기록"입니다

방의 발언 기록은 Orca가 저장하고 **앱을 재시작해도 남습니다**(1:1 대화와 다른 점입니다).
다만 그것은 **누가 무슨 말을 했나**의 기록이고, 각 봇이 그 답에 이르기까지 한 작업은 여전히
그 봇의 전사에 있습니다. 최근 96줄까지 유지됩니다.

봇을 삭제해도 **방은 지워지지 않습니다.** 그 봇만 멤버에서 빠집니다.
...
| | 상태 |
| --- | --- |
| 재시작을 넘기는 1:1 대화 | ⚠️ 데몬이 세션을 살려 두면 이어지고, 아니면 새 대화가 열립니다. 사이드바의 라우팅 기록은 재시작 시 사라집니다. (그룹 방의 발언 기록은 저장되어 남습니다 — §8.7) |
| 방 안에서 봇들이 **동시에** 답하기 | ❌ 한 번에 한 명씩 순서대로 돕니다(§8.7). 동시 실행은 방 기록의 순서를 무너뜨립니다 |
| 방을 프로젝트 너머로 만들기 | ❌ 멤버는 같은 프로젝트여야 합니다 — 다른 체크아웃의 봇은 여기서 띄울 수 없습니다 |
| 방에 파일·이미지 첨부 | ❌ 아직 텍스트만 보냅니다 |
...
### 8.12 역할·설명은 봇에게 실제로 전달됩니다

봇의 **역할**과 **설명**은 화면에만 있는 게 아니라 에이전트에게 **상시 지시**로 들어갑니다.
그래서 설명에 이렇게 적어 두면 그대로 동작합니다:

```
사내 GitLab의 team/api 저장소 안에서만 찾아라. 공개 웹 검색은 쓰지 마라.
기능을 물으면 먼저 README와 docs/ 를 읽고 답하라.
```
...
| 언제 | 역할·설명이 들어가나 |
| --- | --- |
| 대화 첫 메시지 | ✅ (팀메이트 명부도 함께) |
| 다른 봇이 넘긴 일 | ✅ (그 봇의 세션이 처음 뜰 때) |
| **예약 루틴** | ✅ 프롬프트에 함께 저장됩니다 |
| 같은 대화의 후속 메시지 | ❌ 필요 없습니다 — 세션에 이미 있습니다 |

⚠️ **루틴은 만들 때의 역할·설명을 프롬프트에 굽습니다.** 나중에 설명을 고쳐도 **기존 루틴은
바뀌지 않습니다** — 자동화 페이지에서 그 프롬프트를 직접 고치세요. (사용자가 프롬프트를 이미
손봤을 수 있어 자동으로 덮어쓰지 않습니다.)

⚠️ 봇이 실제로 URL·저장소에 **접근할 수 있는지**는 그 에이전트의 도구와 사내 망 정책에
달려 있습니다. Orca는 지시를 전달할 뿐, 에이전트의 네트워크 접근을 대신 열어 주지 않습니다.
