...
npm install -g pnpm   # corepack enable 은 하지 마십시오 — docs/reference/pnpm-12-corepack-install.md
...
현재 `enterprise/samsungds`에는 **`v1.4.207`** 가 병합되어 있습니다(`git log --oneline --merges -3`로 확인). v1.4.159부터 v1.4.184까지 매번 **병합(merge)** 으로 올렸습니다 — 강제 푸시가 필요 없고, 사내에서 이미 받아 간 커밋이 재작성되지 않습니다.
