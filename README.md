...
- 사용자 이름이 비어 있으면 토큰이 `Authorization: Bearer` 로, 채워져 있으면 `Authorization: Basic` 으로 나갑니다. **이 PC에만** 저장되고 OS가 지원하면 암호화됩니다
...
> 토큰 문제가 아니라 **주소 문제**입니다. 그래서 따라가지 않고 갈 주소를 알려 줍니다.

> 💡 **위키 페이지 주소를 그대로 붙여넣어도 됩니다.** `…/display/TEAM/문서`,
> `…/pages/viewpage.action?pageId=…` 같은 주소를 넣으면 앞부분(기지 주소)만 남기고 잘라 냅니다.

---

## 8. 봇(에이전트 팀) 검토 — 만들지 않기로 했습니다

> **결론 (2026-08-25).** 역할 봇 레인을 이 포크에 구현하지 않습니다. 조사해 보니 우리가 설계한 것의
> 대부분이 **Claude Cowork에 이미 있고**, 남는 차별점 하나(역할별 승인 경계를 가진 봇 로스터)를 얻자고
> 나머지를 새로 만드는 비용이 맞지 않습니다. 이 절은 **무엇을 설계했고, 왜 접었고, 언제 다시 볼지**를
> 남기기 위한 기록입니다. 같은 조사를 반복하지 마세요.

### 8.1 조사로 확립한 "봇"의 정의

Nous Research의 hermes-agent **Bot Mode**와 xAI **Grok Bot**을 읽고, Orca의 프리미티브에 맞춰
정리한 모델은 다음 여섯 줄입니다.

> 1. 봇은 **역할과 권한을 가진 이름**이다. 특정 프로젝트에 속하지 않는다.
> 2. 일을 시키면 **서브 에이전트를 띄워 워크트리에서 일하게 하고 끝나면 회수**한다.
> 3. 봇이 다른 봇을 부르는 것은 **권한 상승**이므로 사람을 거친다.
> 4. 예약 실행과 승인 경계는 **(봇 + 프로젝트)** 에 붙는다.
> 5. **프로젝트가 자기 자신을 설명한다** — 문서·게이트 명령·실행 런타임·에이전트 기본값.
> 6. **대화형 작업은 즉흥으로 되지만, 예약 실행은 스킬을 요구한다.**

핵심은 **봇이 런타임이 아니라 이름**이라는 것입니다. 자기 프로세스도, 자기 스케줄러도, 자기 대화
저장소도 갖지 않고 Orca가 이미 가진 것 위에 얹힙니다 — `src/main/runtime/orchestration/`(워커
디스패치·권한·정산), 워크트리, `src/main/automations/`, `skills/`,
`src/renderer/src/components/browser-pane/`, 그리고 §4의 정책 게이트.

역할을 나누는 이유는 성격이 아니라 **권한**입니다. 이게 이 설계에서 유일하게 새로웠던 부분입니다.

| 봇 | 묻지 않고 해도 되는 것 | 반드시 물어야 하는 것 |
| --- | --- | --- |
| 조율자 | 작업 분해 · 배정 · 결과 취합 | **코드 편집 전면 금지** (직접 고치지 않음) |
| 분석가 | 저장소 읽기 · 검색 | **쓰기 전면 금지** |
| 구현자 | 워크트리 파일 편집 · 로컬 커밋 | **push · PR · 머지** |
| 검증자 | lint · typecheck · test · build 실행 | **코드 편집 금지** |
| 리포터 | `reports/` 아래 문서 쓰기 | **소스 편집 금지** · 외부 전송 |

### 8.2 어떻게 동작할 예정이었나

프로젝트 셋(React 프론트엔드 / C# 윈도우 앱 / Electron+React)에 역할 봇 다섯을 놓고 시나리오를
그렸을 때 드러난 동작입니다.

| 국면 | 동작 |
| --- | --- |
| 일 맡기기 | 조율자에게 자연어로. 요청의 마지막 요소는 **리뷰 지점**("어디서 멈춰 물을 것인가") |
| 계획 | 조율자가 작업 DAG를 먼저 제시하고 **쓰기가 포함되면 승인을 받은 뒤** 워커를 띄움 |
| 실행 | 작업마다 워크트리 하나, 서브 에이전트 하나. 끝나면 회수 |
| 관전 | 작업 행을 클릭하면 그 워커의 터미널/챗 뷰가 열림 (Orca에 이미 있음) |
| 막힘 | 2FA·자격증명·CAPTCHA에서 **우회하지 않고 멈추고** 사람을 부름. 사람이 **그 단계만** 처리하고 봇이 이어받음 |
| 승인 | `git push`·PR·머지 등에서 `이번만 / 항상 허용 / 거부`. 승인은 **제안된 행동만** 통제하고 이미 끝난 일을 되돌리지 않음 |
| 인계 | 방에서 봇끼리 떠들지 않고 **파일 경유**(`reports/…`)로 넘김 |
| 산출물 | 보고서·HTML을 Orca 안에서 바로 열기 |
| 반복 | 한 번 해 본 뒤 **스킬**(하는 법)로 굳히고, **루틴**(언제)이 그 스킬을 참조 |

프로젝트마다 언어와 성격이 달라도 봇 하나로 덮이는 이유는 **봇이 언어를 아는 게 아니라 프로젝트가
자기를 설명하기 때문**입니다 — 저장소의 `AGENTS.md`/`CLAUDE.md`, 리포 스코프 명령
(`TerminalQuickCommandScope`의 `{ type: 'repo' }`), 그리고 `src/shared/project-execution-runtime.ts`가
프로젝트마다 정하는 실행 런타임(`local-host` / `windows-host` / `wsl`). C# 윈도우 앱을 Windows
호스트에서 빌드해야 한다는 사실은 봇이 아니라 **프로젝트가 압니다.**

### 8.3 만들지 않기로 한 이유

설계한 것의 대부분이 이미 다른 제품에 있습니다.

| 우리가 설계한 것 | Claude Cowork |
| --- | --- |
| 역할별 승인 경계 | **폴더 단위 권한** — 블랭킷 디스크 접근 없음, 폴더마다 승인·언제든 회수 |
| 루틴(예약 실행) | **있음** — "매주 월요일 9시" 같은 자연어 주기 |
| 프로젝트가 자기를 설명 | **있음** — 워크스페이스 루트 + 하위 폴더별 `CLAUDE.md` |
| 서브 에이전트를 띄우고 회수 | **있음** — Managed Agents, "Claude가 Claude를 부르는 전문 서브에이전트" |
| 막히면 사람을 부름 | **있음** — 사용자만 내릴 수 있는 결정에서 입력 요청 |
| 스킬 | **있음** — 스킬·플러그인·커넥터 마켓플레이스 |
| 산출물은 리뷰용 초안 | **있음** — 고위험 업무는 사람 승인을 전제로 설계 |

거기에 Cowork에는 우리가 설계하지도 않은 것이 더 있습니다 — 웹훅 트리거, 모바일·웹, 노트북을
닫아도 도는 백그라운드 실행, 업종별 번들. **확인된 범위에서 Cowork에 없는 건 이름 붙은 역할 봇
로스터 하나뿐입니다.**

Orca가 갈라지는 지점은 셋이고, **셋 다 조건부**입니다.

1. **코드 저장소 작업.** Cowork는 폴더에 접근하지 워크트리를 만들거나 브랜치를 관리하지 않습니다.
   "워크트리에서 고치고, 로컬 커밋까지만 하고, `git push`에서 멈춘다"는 Cowork 모델에 자리가 없습니다.
   (Anthropic 자체 집계로 Cowork 세션 120만 건 중 90% 이상이 소프트웨어 개발과 무관했습니다.)
2. **실행 위치.** 사내 빌드 머신에서 돌아야 하는 작업(예: C# 윈도우 앱)은 Cowork가 붙지 못합니다.
   Orca는 §0·§4의 런타임 모델과 SSH 릴레이로 이미 그걸 합니다.
3. **폐쇄망.** Cowork 기본 인프라는 US 호스팅이고 데이터 레지던시 선택은 상위 등급 전용입니다.
   이 포크가 존재하는 이유(§4)와 정면으로 만나는 지점입니다.

**그럼에도 만들지 않는 이유**는, 위 셋이 참이라 해도 그것을 얻자고 로스터·승인 UI·작업 보드·
스킬/루틴 분리를 전부 새로 만드는 비용이 크고, 특히 **비(非)코드 업무(메일 정리·보고서·일정)로
범위를 넓히는 순간 Cowork에 명확히 밀리기 때문**입니다. 커넥터 생태계·모바일·백그라운드 실행이
전부 그쪽이 앞섭니다.

### 8.4 이 결정을 다시 볼 조건

아래 중 하나가 참이 되면 재검토할 가치가 있습니다.

- **사내 소스가 외부 클라우드로 나갈 수 없다는 정책이 확정**되고, 동시에 팀이 여러 저장소에 걸친
  반복 작업(게이트 실행 → 보고 → 승인 후 반영)을 실제로 자주 한다.
- Orca 오케스트레이션(§7)을 손으로 쓰는 빈도가 높아져, **워커 기동·승인·결과 회수를 매번 CLI로
  하는 것이 병목**이 된다. 그때 봇 레인은 "새 기능"이 아니라 **이미 하는 일에 얼굴을 붙이는 일**이
  되므로 비용이 크게 줄어듭니다.
- 승인 경계를 **관리자가 중앙에서 강제**해야 할 요구가 생긴다(§4의 정책 파일과 같은 층).

반대로 **비코드 업무 자동화(메일·문서·일정)를 원한다면 이 레인을 되살리지 말고 Cowork를
검토하세요.** 그 방향에서는 우리가 이길 수 없습니다.

### 8.5 이력 주의 — 되돌려진 구현이 남아 있습니다

이 저장소 이력에는 **봇 레인 구현 커밋 약 20건과 그 revert(`c985f2b2d9`, 92파일 `-12,163`줄)** 가
있습니다. `git log --all --grep=bot -i`로 발견할 수 있습니다.

⚠️ **그 구현을 되살려 출발점으로 삼지 마세요.** UX 모델이 위 §8.1과 다릅니다 — 봇이 프로젝트에
바인딩돼 있었고(`workspaceKey`/`projectId`), 서브 에이전트 수명주기 대신 봇끼리 직접 위임하는
구조였습니다. 되살리려면 §8.1의 여섯 줄부터 다시 설계해야 합니다.

### 8.6 조사에 쓴 자료

- **hermes-agent Bot Mode** — `apps/desktop/src/plugins/hermes-bots/plugin.js` (11,700줄) 및 테스트 54건
- **xAI Grok Bot** — [공식 발표](https://x.ai/news/introducing-grok-bot),
  [문서](https://docs.x.ai/grok-bot/bots) (봇 관리 · 스킬/루틴 · 컴퓨터와 앱 · 승인/보안 · 사용 사례),
  [실사용 독립 리뷰](https://flaviocopes.com/grok-bot/)
- **Claude Cowork** — [TechCrunch (2026-07-07)](https://techcrunch.com/2026/07/07/the-coding-agent-wars-are-spilling-into-the-rest-of-the-office-claude-cowork/),
  [기능·한계 가이드](https://techsy.io/en/blog/claude-cowork-guide)

> 상세 분석 노트는 임시 디렉터리에 작성했고 커밋하지 않았습니다. 위 절이 그 결론을 담은 유일한 기록입니다.
