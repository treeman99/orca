...
| `disableUnattendedAgentRuns` | boolean | `lockdown` | **Orca 자신의 스케줄러**를 거부합니다 — 예약된 자동화와 봇 루틴이 시간이 되어도 에이전트를 띄우지 않고, 실행 기록만 `skipped_policy`로 남습니다. `disableExternalAutomations`와 **다른 축**입니다: 저쪽은 벤더 CLI(`hermes`/`openclaw`) 레인이고, Orca 내장 60초 틱 스케줄러(`src/main/automations/service.ts`)에는 그 전까지 정책 게이트가 **한 곳도 없었습니다** — `{"lockdown": true}`만으로도 사내 기계에서 무인 에이전트 실행이 계속 돌 수 있었다는 뜻입니다. 게이트 1곳(`src/main/enterprise/unattended-agent-run-guard.ts`의 `unattendedAgentRunRefusal`, 호출 지점은 `AutomationService.requestDispatch`)이 60초 틱과 헤드리스 `orca serve` 디스패처를 함께 덮습니다 — 워크스페이스 해석보다 **앞**에 있어 거부된 실행은 들여다본 흔적도 남기지 않습니다. ⚠️ **"지금 실행"(수동 트리거)은 일부러 남깁니다** — 키보드 앞에 사람이 있고, 같은 프롬프트를 터미널에 직접 칠 수 있으므로 막아도 위험이 줄지 않습니다. ⚠️ 번들 정책(`resources/enterprise-policy.json`)은 이 키를 **명시적으로 `false`**로 둡니다: 스위치 도입만으로 이미 쓰던 예약 자동화가 조용히 꺼지면 안 되기 때문이며, 무인 실행을 막으려면 그 줄을 지우거나(=lockdown 상속) `true`로 바꾸세요. 동작 테스트 `src/main/automations/service-enterprise-policy.test.ts` |
...
| `disableRemoteOrcaServer` | boolean | `lockdown` | 이 데스크톱이 **다른 Orca에 붙는 것**(아웃바운드)을 거부합니다 — 설정 → 원격 Orca 서버, 페어링 코드 등록, ephemeral VM, 부팅 시 저장된 원격을 활성 런타임으로 복원하는 hydration. **SSH 호스트와 인바운드 `orca serve` 리스너는 대상이 아닙니다** — 원격 개발을 통째로 없애지 않습니다. 게이트: `src/main/ipc/runtime-environment-transport-routing.ts`의 status/call/subscribe 3개 진입점 (`:68`, `:144`, `:252`) + IPC 핸들러 3곳 (`src/main/ipc/runtime-environment-connectivity-handlers.ts`의 `addFromPairingCode`·`verifyAndAddFromPairingCode`·`retryControlConnection`). **마지막 것은 아웃바운드 소켓을 열지 않습니다** — 캐시에 이미 있는 연결만 재시도하므로 위 3개 진입점이 이미 덮고 있지만, 그 안전성이 "캐시는 게이트 뒤에서만 채워진다"는 업스트림 불변식에 의존해 v1.4.195에서 핸들러 자체에도 걸었습니다 (`remote-orca-server-retry-control-connection-policy.test.ts`, 뮤테이션 검증 완료) |
...
| `allowedAgents` | string[] | `null` (제한 없음) | 사용자가 **쓸 수 있는 에이전트 CLI id 목록** (예: `"claude"`). ⚠️ **UI 필터가 아니라 하드 거부입니다** — 예전 판의 "고를 수 있는 목록"이라는 설명은 부족했습니다. 세 축으로 동작합니다: ① **표시** — 에이전트/모델 피커·계정 설정·하단 사용량 미터가 이 목록으로만 좁혀지고, 나머지 벤더(codex/gpt, gemini, opencode, grok 등)는 UI에서 사라지고 사용량 폴링(예: Codex → chatgpt.com)도 하지 않습니다. ② **스폰 거부** — 목록 밖 에이전트는 **실제로 실행되지 않습니다**. 표시 게이트만으로 부족한 이유는 렌더러를 거치지 않는 경로가 여럿이기 때문입니다: 정책 배포 전에 바인딩된 키보드 코드, `orca` CLI, 페어링된 모바일/웹 클라이언트, 오케스트레이션 디스패치. 초크포인트 2곳 — `pty:spawn` IPC(`src/main/ipc/pty.ts`)와 `runtime.setPtyController({spawn})`(`src/main/runtime/orca-runtime.ts`, CLI·모바일·자동화·오케스트레이션이 지나는 레인). 거부는 `agent_blocked_by_enterprise_policy` 오류로 사용자 토스트에 표시됩니다 (`src/main/enterprise/agent-allowlist-guard.ts`). ③ **managed hook 설치/스윕** — 목록 밖 에이전트의 **훅 런처를 새로 깔지 않고, 이미 깔린 것을 다음 실행에 지웁니다.** ①②만으로는 실행조차 못 하는 에이전트의 훅 스크립트가 `~/.orca/agent-hooks/`와 사용자 홈의 벤더 설정 파일에 그대로 남아 있었습니다. 초크포인트는 `applyAgentStatusHooksEnabledUnderEnterprisePolicy`(`src/main/agent-hooks/enterprise-agent-hook-policy.ts:21`) 한 곳이고, 부팅(`src/main/index.ts:2907`)·런타임 재조정(`src/main/runtime/orca-runtime.ts:3938`)·설정 IPC(`src/main/ipc/settings.ts:213`) 세 진입점이 전부 그리로 들어갑니다. 설치 쪽은 차단 목록을 데이터로 받아 걸러내고(`managed-agent-hook-controls.ts:31`의 `blockedAgents`), 걸러진 에이전트는 조용히 빠지는 게 아니라 `skipReason: 'agent_blocked_by_policy'`로 **보고**됩니다(`:144`·`:156`, 값은 `src/shared/agent-hook-types.ts:29`) — 사용자가 끈 `agent_disabled`와 구분해야 하기 때문입니다. 스윕은 `sweepEnterpriseBlockedAgentHooks()`(`src/main/agent-hooks/enterprise-agent-hook-sweep.ts:100`)이고 지우는 대상은 두 가지입니다: **(a)** 각 에이전트 자기 설정 파일의 Orca managed 항목(`~/.gemini/settings.json`, `~/.cursor/hooks.json`, `~/.factory/settings.json` 등 — `remove()` 경유, `:75`), **(b)** `~/.orca/agent-hooks/<agent>-hook.{sh,cmd,ps1}` 런처와 같은 폴더의 `<agent>-` 접두 파일 전부(Antigravity의 이벤트별 `.cmd` 래퍼, Claude/OpenClaude의 statusline 스크립트 — `:88`). **순서는 (a) 먼저, (b) 나중이며 뒤집으면 안 됩니다** — 런처를 먼저 지우면 설정이 존재하지 않는 스크립트를 계속 호출해 훅 이벤트마다 에러가 납니다. (a)는 **이미 설치돼 있을 때만** 돕니다(`:62`의 `hasManagedHooksInstalled`) — 그러지 않으면 정리 코드가 설치된 적도 없는 `~/.gemini/` 같은 벤더 설정 디렉터리를 **새로 만들어 버립니다**. `allowedAgents`가 `null`(정책 없음)이면 스윕은 **첫 줄에서 그대로 반환합니다**(`:101`) — 업스트림 빌드와 `pnpm dev`는 영향이 없습니다. ⚠️ **잔여 위험 1 — 원격/SSH·WSL 호스트는 새 설치만 막고, 이미 깔린 원격 런처는 지우지 않습니다** (`src/main/agent-hooks/remote-managed-hook-installers.ts:99`). 원격 파일 스윕은 구현돼 있지 않으므로, 정책 배포 전에 원격 호스트에 깔린 훅은 그 호스트에서 직접 지워야 합니다. ⚠️ **잔여 위험 2 — 데스크톱 앱이 꺼진 상태의 `orca agent hooks enable`은 게이트되지 않습니다** (`src/cli/handlers/agent-hooks.ts:199`의 오프라인 분기). 앱이 떠 있으면 런타임 RPC로 가서 위 초크포인트를 지납니다. 오프라인으로 깔린 훅은 다음 앱 실행의 스윕이 지웁니다(자가치유). 동작 테스트: `src/main/agent-hooks/managed-agent-hook-policy-gate.test.ts`(설치 게이트·원격 게이트), `enterprise-agent-hook-sweep.test.ts`(삭제 순서·멱등성), `enterprise-agent-hook-sweep-side-effects.test.ts`(빈 홈에 아무것도 만들지 않음 — 실제 서비스로 검증). ④ **structured 에이전트 세션 레인** — v1.4.193이 `agentSession.*` RPC 19개를 들여왔고, 그 codex 레인은 **PTY가 아니라 `spawnProcess` 자식 프로세스**로 `codex app-server`를 띄웁니다. ②의 초크포인트는 둘 다 PTY라 이 레인을 보지 못하므로 별도 초크포인트 2곳을 세웠습니다: **(a)** attach funnel `attachStructuredAgentSession()`(`src/main/native-chat/agent-session-wire/structured-agent-session-attach-orchestration.ts`) — `agentSession.create`의 두 분기(create-by-intent / create-by-attach-params), `agentSession.ensure`, 그리고 surface-hold 재개가 전부 이 한 호출로 모입니다. 세션이 예약되기 **전**에 거부하므로 레코드·저널·멱등성 행이 남지 않고, attach 큐에도 들어가지 않습니다. 이 함수는 Promise를 반환하지만 `async`가 아니라서 동기 throw가 호출자의 `.catch`를 빠져나가므로 `agentBlockedByPolicyError()`로 **reject** 합니다. **(b)** 스폰 직전 `createCodexStructuredLaunchResolver()`(`src/main/codex/codex-structured-launch-resolution.ts`) — 스토어에 이미 있는 레코드는 **다시 attach 하지 않고** 여기서 바로 재기동됩니다(wake·소유자 교체·재시작 복원). 정책이 배포되기 전에 만들어진 세션이 만나는 유일한 거부이자, 벤더 CLI가 자식 프로세스가 되기 직전의 마지막 지점입니다. 여기에 더해 두 곳이 거부를 **미리** 알려 줍니다: `getStructuredAgentSessionCreateSupport()`는 기존 값 `reason: 'agent'`로 `{ supported: false }`를 돌려주고(신규 wire 값 없음), 차단된 에이전트가 structured 호스트를 존재하게 만드는 것도 막습니다 — 다만 이 프로브는 **현재 이 저장소 안에 호출자가 없습니다**(원격/모바일 클라이언트를 위한 표면). 데스크톱 렌더러는 프로브 없이 `agentSession.create`로 바로 가므로, 진입점을 실제로 없애는 것은 `canUseStructuredNativeChat()`(`src/renderer/src/lib/structured-native-chat-availability.ts`)의 `allowedAgents` 좁히기입니다 — 이것이 없으면 **열 수 있다고 제시한 탭에서 눌렀을 때 에러 토스트**가 납니다. ⚠️ `enforceNetworkAllowlist`는 이 레인을 구제하지 못합니다 — 자식 프로세스는 Electron 세션도 메인 프로세스 `fetch`도 아닙니다(§5). 동작 테스트: `src/main/runtime/structured-agent-session-enterprise-policy.test.ts`(attach·스폰·프로브)와 `src/renderer/src/lib/structured-native-chat-availability.test.ts`(렌더러 좁히기). **네 게이트를 하나씩만 임시 제거해 각자의 케이스만 빨개지는 것을 확인했습니다**(attach 2건 / 스폰 1건 / 프로브 1건 / 렌더러 1건). 빈 배열/오타는 피커를 완전히 막지 않도록 "제한 없음"으로 처리됩니다. 사내 self-hosted 모델은 에이전트가 아니라 허용된 에이전트의 모델 피커에 얹히므로 여기 적을 필요가 없습니다. §3-3 참고 |
| `allowedModels` | string[] | `null` (제한 없음) | 허용된 에이전트 **안에서** 고를 수 있는 **모델 id 목록** (예: `["opus","sonnet"]`). ⚠️ **이건 프로브로는 풀 수 없는 문제입니다** — 모델 피커는 에이전트 **바이너리가 아는** 목록을 냅니다. 사내 게이트웨이가 그 모델을 실제로 서비스하는지는 별개 사실이라, 턴 도중 거부되기 전까지 드러나지 않습니다. 그래서 게이트웨이가 좁은 플릿에서도 `fable` 같은 항목이 계속 보였습니다. 매칭은 **접두사**입니다: `"opus"` 하나로 `opus`·`opus[1m]`·`claude-opus-5`가 모두 덮입니다 — CLI 별칭과 정확한 이름은 같은 모델이고, 플릿이 철자를 전부 나열할 이유가 없습니다. 적용 지점은 채팅 모델 피커(`native-chat-pty-session-options.ts`)입니다. 오타로 아무것도 안 남는 목록은 **무시**합니다 — 빈 피커는 앱이 고장 난 것이기 때문입니다. `lockdown`을 상속하지 않습니다 |
| `updateReleaseRepository` | string | `null` → 빌드 상수 `daegun-kim/Orca_ds` | §3-2의 릴리스 조회가 볼 저장소를 `OWNER/REPO`로 지정합니다. **`lockdown`을 상속하지 않습니다** — 스위치가 아니라 값이라, `githubEnterpriseHost`·`allowedNetworkHosts`와 같은 취급입니다. 조회를 끄는 것은 `disableAutoUpdate`이지 이 키가 아닙니다. **URL도 호스트도 받지 않습니다**: 릴리스 페이지 링크를 통째로 붙여 넣어도 형식 검사에서 걸러져 경고가 나고 빌드 기본값이 그대로 섭니다 — 조회 대상 호스트는 언제나 `githubEnterpriseHost`이고 이 키가 그것을 바꿀 수는 없습니다 (`readRepositoryCoordinate`, `src/shared/enterprise-policy.ts`) |
| `$schema` | string | — | 알려진 키라 경고가 나지 않습니다. 에디터 편의용 (`enterprise-policy.ts:180`) |

### 3-0. 정책이 아니라 코드에서 제거된 것

아래 표면은 **정책 파일 유무와 무관하게** 이 포크의 바이너리에 존재하지 않습니다. 스위치를 어떻게 두든 되살아나지 않습니다.

> **Artifacts 공유와 에이전트 스킬 공유(v1.4.188)도 여기에 속합니다 — 이 둘을 켜고 끄는 정책 키는 없고, 앞으로도 만들지 않습니다.** 관리자가 `enterprise-policy.json`에서 찾을 필요가 없다는 뜻입니다. 이유는 두 가지입니다. ① 목적지가 벤더 SaaS로 코드에 잠겨 있어(`onorca.dev` / `storage.googleapis.com`) 사내 호스트로 돌릴 수 없으므로, "켠다"는 선택지가 애초에 사내 배포에서 성립하지 않습니다 — 스위치를 두면 그 선택지를 제공하는 셈이 됩니다. ② 스킬 공유의 설치 레인은 **SSH 릴레이(원격 호스트)에서도 실행**되는데, `getEnterprisePolicy()`는 `electron`을 import하므로 릴레이 번들에 들어갈 수 없고 정책 파일도 원격 호스트에 배포되지 않습니다. 그 호스트에서 정책은 항상 `lockdown: false`로 해석되므로, **정책 스위치는 이 레인에서 물리적으로 동작하지 않습니다.** `src/shared/skill-sharing-removal.ts`의 컴파일타임 상수만이 가드하는 코드와 함께 릴레이 번들에 실립니다. 범위와 검증 방법은 [외부 연동 감사 §3.1](./external-integrations-audit.md)에 있습니다.
...
- **managed hook 설치/스윕 (메인):** 허용되지 않은 에이전트는 훅 런처를 **새로 받지 않고**, 이미 깔린 것은 앱이 뜰 때 지워집니다 — 설정 파일의 managed 항목을 먼저 정리하고 그다음 `~/.orca/agent-hooks/`의 런처를 지웁니다. 초크포인트 1곳: `applyAgentStatusHooksEnabledUnderEnterprisePolicy`(`src/main/agent-hooks/enterprise-agent-hook-policy.ts:21`). ⚠️ **원격/SSH·WSL 호스트는 새 설치만 막고 이미 깔린 원격 런처는 지우지 않습니다.** 자세한 범위와 잔여 위험은 §3 표의 `allowedAgents` 행을 보세요.
...
```jsonc
{
  ...
  // 새 버전 알림이 볼 저장소. 생략하면 빌드 상수 "daegun-kim/Orca_ds"가 그대로 쓰입니다 (§3-2).
  // 알림 자체를 켜려면 아래 disableAutoUpdate를 false로 두십시오.
  // "updateReleaseRepository": "daegun-kim/Orca_ds",
  ...
  "disableUnattendedAgentRuns": false,   // Orca 자신의 예약 스케줄러 (봇 루틴 포함). 기본 false — 이유는 §3 스키마 표
...
| `disableUsagePolling` | 설정 사이드바, Cmd+J에 `통계` / `usage`, 상태바 사용량 칩 | **설정 → 통계 및 사용량 팬이 사라집니다** (사이드바·Cmd+J·설정 검색 전부). 상태바 팝오버의 "Usage details & history" 항목도 없어집니다. 칩 자체는 남되 영구 스피너 없이 `unavailable` 상태 (`rate-limits/service/service-result-policy.ts:99`) |
...
| `disableUnattendedAgentRuns` | 사이드바 → 자동화, 사이드바 → Bots | 예약된 자동화와 봇 루틴이 **더 이상 시작되지 않고** 실행 기록에 `Blocked by policy`가 남습니다. 봇 상세의 `+`(새 루틴) 버튼이 사라지고 대신 정책 안내 문구가 표시되며, **이미 만들어진 루틴은 목록에 그대로 보입니다**(사라지면 왜 안 도는지 알 수 없습니다). ⚠️ 자동화 페이지의 **"지금 실행"은 그대로 동작해야 정상입니다** — 이 스위치는 타이머에 관한 것입니다 |
