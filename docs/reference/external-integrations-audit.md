...
>
> **새 인바운드 표면 1건 — `src/main/orcad/**` (egress 아님).** Electron 없이 plain Node에서 Orca 런타임을 띄우는 데몬이고, `enableWebSocket: true`로 `OrcaRuntimeRpcServer`를 띄웁니다. **나가는 연결이 아니라 받는 리스너**이며, 그 RPC 서버는 v1.4.188의 `orca serve`가 쓰던 것과 같은 코드입니다(`src/main/runtime/runtime-rpc.ts`의 트리 diff 0줄, 바인드 상수도 동일). **데스크톱 인스톨러에 포함되지 않습니다** — electron-builder 설정에 `orcad` 참조가 없고 `build:orcad`/`smoke:orcad-terminal` 스크립트와 CI에서만 쓰입니다. 사내 배포 표면이 아니므로 이번 판의 판정 대상이 아니지만, **`pnpm build:orcad` 산출물을 배포할 계획이 생기면 그 시점에 정책 적용 여부를 새로 판정해야 합니다** — `disableRemoteOrcaServer`는 데스크톱 `--serve`를 덮을 뿐 별도 바이너리를 덮지 않습니다.
>
