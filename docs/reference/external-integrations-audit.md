...
>
> 🔴 **그 대신 기존 목적지로 가는 새 문이 하나 열렸습니다 — §0.2 #31.** `agentSession.*` RPC 19개가 전부 신규이고(v1.4.192에는 0개), 그 codex 레인은 `codex app-server`를 **PTY가 아니라 `spawnProcess` 자식 프로세스**로 띄웁니다. 이 포크의 `allowedAgents` 강제 10곳은 전부 PTY 스폰과 구(舊) 에이전트 세션 RPC(`ensureAgentSession`/`createAgentSession`)에 걸려 있고, **structured 레인에는 정책 검사가 0건**입니다. 새 목적지가 아니라 **도달 경로**의 문제이므로 신규 스위치가 아니라 기존 `allowedAgents`를 새 초크포인트에 거는 것이 해법입니다.
...
| 31 | **structured 에이전트 세션 → `codex app-server` 자식 프로세스** (egress가 아니라 **허용목록 우회 경로** 항목입니다) | codex CLI가 정하는 목적지(OpenAI). Orca는 목적지를 모릅니다 — #1과 같은 자식 프로세스 레인 | `agentSession.create` / `agentSession.ensure` RPC 호출 시. 렌더러에서는 `experimentalStructuredNativeChat` + `openAgentTabsInChatByDefault` 두 설정이 켜져야 하고 win32는 `canUseStructuredNativeChat`이 거부하지만, **그 판정은 렌더러에만 있습니다** — `orca` CLI·페어링된 모바일/웹 클라이언트·orchestration 디스패치·원격 클라이언트는 렌더러를 지나지 않고, 메인 쪽 지원 판정(`supportsCodexStructuredLocation`)은 로컬·비WSL이면 Windows도 통과시킵니다(프로세스 생성시각을 읽을 수 있을 때 — 이 포크는 패치된 `@vscode/windows-process-tree`를 출하하므로 사내 Windows 플릿에서 통과할 수 있습니다) | 🔴 **v1.4.193 신설이며 아직 게이트가 없습니다.** 사내 정책은 `allowedAgents: ["claude", "opencode"]`이고 빌드 내장 바닥값도 같으므로 **codex는 허용되지 않습니다**(`resources/enterprise-policy.json:53`, `src/shared/enterprise-policy-baseline.ts:60`). 그런데 이 포크의 `assertAgentAllowedByEnterprisePolicy` 호출 10곳은 전부 **PTY 스폰**과 **구(舊) 에이전트 세션 RPC**(`ensureAgentSession`/`createAgentSession`)에 있고, structured 레인은 PTY가 아니라 `spawnProcess` JSONL stdio로 뜨므로 그 어느 것도 지나지 않습니다 — structured 표면 전체에서 정책 개념 검색이 **0건**입니다. upstream이 새로 넣은 `structured-agent-session-gate.ts`는 **클라이언트 capability 협상**(`agent-session.structured.v1` 광고 여부)만 보며 관리자 정책과 무관합니다. `enforceNetworkAllowlist`로도 구제되지 않습니다(#1과 동일한 자식 프로세스 사각지대). **신규 스위치는 불필요합니다** — 기존 `allowedAgents`가 정확히 이 의미("어떤 CLI를 띄울 수 있는가")를 가지므로, 초크포인트에 걸기만 하면 됩니다. 후보: ① RPC 진입점 `structured-agent-session-gate.ts`(거부가 RPC 오류로 표면화됨), ② 스폰 직전 `codex-structured-launch-resolution.ts`의 resolver(진입점과 무관하게 막힘 — 진짜 초크포인트). **둘 다 거는 것을 권고하며, 뮤테이션 검증(게이트를 임시로 지워 테스트가 빨개지는지)을 반드시 하십시오** | 신규 RPC `src/main/runtime/rpc/methods/structured-agent-session.ts`, capability 전용 게이트 `structured-agent-session-gate.ts`, 정책 없는 생성 경로 `src/main/runtime/orca-runtime.ts`의 `resolveStructuredAgentSessionCreateIntent`/`resolveStructuredAgentSessionIntent`, 스폰 `src/main/codex/codex-structured-launch-resolution.ts:59` → `codex-app-server-connection.ts:1,67`, 도달 판정 `src/main/codex/codex-structured-location-support.ts:5`, 렌더러 전용 거부 `src/renderer/src/lib/structured-native-chat-availability.ts`, 대조군(게이트가 있는 구 레인) `orca-runtime.ts:30444`·`:30539` |
