...
>
> **v1.4.190 갱신 (2026-08-27).** 이 판에서 재검증한 것은 **v1.4.188 → v1.4.190 구간의 델타**입니다(트리 diff 819 파일, +46,045/−14,149, 신규 244건 · 삭제 0건). 판정은 양 태그의 **프로덕션 네트워크 호출지점 집합 대조**(라인 문자열 정렬 비교) + 스킴 URL 호스트 집합 차집합 + 추가 라인 46,018줄 전수 호스트 히스토그램 + `openExternal`·IPC 채널·릴레이 RPC·CLI 커맨드·`GlobalSettings` 키·의존성·자식 프로세스 실행 파일 차집합 + **신규 프로덕션 파일 149건 전문 스캔** + `.github/` 차집합입니다. 나머지 확정 항목은 각자의 기준 시점 서술이며 이번에 다시 열어보지 않았습니다.
>
> **신규 외부 목적지 0건.** 프로덕션 호출지점은 65 → 71줄이지만 실 증가는 2건이고, 둘 다 **Jira 요청의 `net.fetch`를 주입 가능한 HTTP 포트로 바꾼 리팩터**입니다 — `src/main/network/http-client.ts`(Node 기본값, `globalThis.fetch` 위임)와 `src/main/host/electron-http-client.ts`(데스크톱, `net.fetch` 위임). 목적지도 도달 조건도 그대로이고(사용자가 설정한 Jira 사이트), 두 구현 모두 기존 `enforceNetworkAllowlist`의 두 레인 안에 정확히 들어옵니다 — 전자는 `installFetchGuard()`가 래핑한 `globalThis.fetch`를 **호출 시점에** 읽고, 후자는 `installSessionGuard()`가 붙은 `session.defaultSession`을 씁니다. 나머지 축: 스킴 URL 호스트 402 → 402(신규는 테스트 픽스처 `evil.test` 하나, 오히려 `docs.anthropic.com`이 빠짐), `openExternal` 83 → 83, 릴레이 RPC 120 → 120, `GlobalSettings` 키 205 → 205, IPC 채널 신규 0 · 제거 1, 자식 프로세스 실행 파일 신규 0 · 제거 2, `.github/` 신규 외부 이미지·액션·패키지 소스 0건.
>
> **v1.4.188 판이 위에서 예고한 계수법 사각지대를 닫았습니다.** 위 표준 정규식에 더해 `\?\? *fetch`/`\?\?= *fetch`/`fetcher!\(`/`deps.fetch`/`httpClient.fetch`/`.fetch(`로 **주입형 fetcher를 따로 스윕**했습니다(26 → 28줄). 증가 3줄·감소 1줄은 위 Jira 포트 리팩터와 정확히 같은 세 파일이고, 숨어 있던 호출지점은 **없었습니다.** 다음 동기화에도 이 패턴을 같이 돌리십시오.
>
> **신규 의존성 1건 — `@vscode/windows-process-tree@0.8.0`(optionalDependencies).** Windows 프로세스 테이블을 Toolhelp32/psapi로 읽는 네이티브 애드온으로 **런타임 네트워크가 없습니다.** 이것이 대체하는 것은 기존의 `powershell.exe`(`Get-CimInstance Win32_Process`) fork 7곳입니다(706ms → 16ms). Windows 패키지 런타임에 동봉됩니다(`config/packaged-runtime-node-modules.cjs`). ⚠️ **미확인**: 이 패키지의 설치 스크립트가 prebuild/node-gyp 헤더를 받으러 나가는지는 저장소 밖 계약이라 소스로 확인할 수 없습니다 — **런타임이 아니라 빌드타임 항목**입니다. 참고로 `package.json`의 `pnpm.onlyBuiltDependencies`에 없어 pnpm이 이 패키지의 빌드 스크립트를 기본 차단합니다(설치 시 "Ignored build scripts" 경고). 폐쇄망 빌드에서 실제로 막히는지는 사내 빌드로만 확정됩니다.
>
> **새 인바운드 표면 1건 — `src/main/orcad/**` (egress 아님).** Electron 없이 plain Node에서 Orca 런타임을 띄우는 데몬이고, `enableWebSocket: true`로 `OrcaRuntimeRpcServer`를 띄웁니다. **나가는 연결이 아니라 받는 리스너**이며, 그 RPC 서버는 v1.4.188의 `orca serve`가 쓰던 것과 같은 코드입니다(`src/main/runtime/runtime-rpc.ts`의 트리 diff 0줄). **v1.4.192에서 바인드가 갈라졌습니다** — orcad는 더 이상 `exposeNetworkByDefault: true`로 뜨지 않고 `resolveOrcadBindHost()`의 기본값 `127.0.0.1`에 묶이며(`src/main/orcad/orcad-bind-address.ts:11,28`), 노출은 운영자가 `--bind 0.0.0.0`을 **명시**해야만 일어나고(호스트명은 거부, 리터럴 IP만) 핀된 바인드는 이후에도 넓어지지 않습니다(`src/main/runtime/runtime-rpc.ts` `refusing to widen to all interfaces`). 즉 v1.4.191까지의 wide 기본값이 사라졌습니다. **데스크톱 인스톨러에 포함되지 않습니다** — electron-builder 설정에 `orcad` 참조가 없고 `build:orcad`/`smoke:orcad-terminal` 스크립트와 CI에서만 쓰입니다. 사내 배포 표면이 아니므로 이번 판의 판정 대상이 아니지만, **`pnpm build:orcad` 산출물을 배포할 계획이 생기면 그 시점에 정책 적용 여부를 새로 판정해야 합니다** — `disableRemoteOrcaServer`는 데스크톱 `--serve`를 덮을 뿐 별도 바이너리를 덮지 않습니다.
>
> **⚠️ 정책 모듈이 놓일 수 있는 자리가 좁아졌습니다.** v1.4.190은 `check:runtime-electron-ratchet`(`config/scripts/check-runtime-electron-ratchet.mjs` + `config/runtime-electron-baseline.txt`)를 `pnpm lint`와 CI에 넣었습니다 — `orca-runtime.ts`/`runtime-rpc.ts`를 esbuild로 번들해 **`electron`을 import하는 모듈이 하나라도 있으면 빌드 실패**이고, 베이스라인은 **비어 있어야** 합니다. 포크의 `enterprise-policy-file.ts`가 `electron.app`을 쓰고 있었으므로 정책을 읽는 런타임 그래프 모듈 전부가 여기에 걸립니다. **베이스라인에 항목을 더하지 않고 포트 뒤로 뺐습니다**: 정책 로더는 `shared/app-environment`(`getPath`/`getAppPath`/`isPackaged`)를 쓰고, Computer Use 승인 다이얼로그는 `RuntimeDesktopSurface.confirmComputerUseAction`으로 옮겼으며(실제 프롬프트는 `host/electron-runtime-desktop-surface.ts`, 창이 없으면 `no-window` = 거부로 fail-closed 유지), 런타임 그래프에서 쓰는 번역은 `i18n/main-translation-port.ts`를 지납니다. **다음 동기화에서 정책 게이트를 새로 놓을 때는 그 파일이 런타임 그래프 안인지 먼저 확인하고, 안이라면 `electron`을 직접 import하지 마십시오.**
>
> ⚠️ **이 구간의 판정도 정적 분석입니다.** 코드 경로로 판정했고 패킷 캡처는 하지 않았습니다.
>
> **v1.4.193 갱신 (2026-09-01).** 이 판에서 재검증한 것은 **v1.4.192 → v1.4.193 구간의 델타**입니다(트리 diff 851 파일, +74,612/−3,386, 신규 419건 · 삭제 0건). 판정은 양 태그의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)(`/`new WebSocket`/`EventSource`/`axios`/`undici`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 스킴 URL 호스트 집합 차집합 + 추가 라인 전수 호스트 히스토그램 + `openExternal`·IPC 채널·RPC 메서드·의존성·`.github/` 차집합 + 신규 파일이 몰린 도메인 3곳(`native-chat` 127건, `crash-reporting` 7건, `ai-vault` 2건) 정밀 검토입니다. **v1.4.191·192 구간은 각 동기화 시점에 "신규 외부 목적지 0건"으로 판정했고 그 기록은 `v1.4.191-samsungds`·`v1.4.192-samsungds` 태그 본문에 있습니다 — 이 문서에는 옮겨 적지 않았습니다.**
>
> **신규 외부 목적지 0건.** 프로덕션 호출지점은 **81 → 81**로 개수뿐 아니라 **집합 자체가 동일**하고(파일 55개도 동일), `openExternal`도 **58 → 58** 집합 동일, 하드코딩 호스트도 `src/` 606개·저장소 683개로 차집합 0, 신규 의존성은 `proper-lockfile@4.1.2`(파일 잠금, 네트워크 없음)와 그 타입 정의뿐입니다. `+48,818`줄이 `src/main`에 들어왔는데 네트워크 호출지점이 한 줄도 늘지 않았습니다 — 신규 코드의 성격이 **직렬화·저널링·런타임 RPC 배관**이기 때문입니다. `src/main/native-chat`(신규 127건)은 이름과 달리 벤더 API를 부르지 않습니다: 네트워크 프리미티브·URL 리터럴·자격증명 참조가 모두 **0건**이고, 실체는 로컬 에이전트 CLI가 디스크에 쓴 transcript를 읽어 디코딩하는 계층입니다. `crash-reporting`은 `uploadToServer: false`인 채 **이 구간에 한 줄도 바뀌지 않았습니다**(`crashpad-capture.ts:68`).
>
> 🔴 **그 대신 기존 목적지로 가는 새 문이 하나 열렸습니다 — §0.2 #31.** `agentSession.*` RPC 19개가 전부 신규이고(v1.4.192에는 0개), 그 codex 레인은 `codex app-server`를 **PTY가 아니라 `spawnProcess` 자식 프로세스**로 띄웁니다. 이 포크의 `allowedAgents` 강제 10곳은 전부 PTY 스폰과 구(舊) 에이전트 세션 RPC(`ensureAgentSession`/`createAgentSession`)에 걸려 있고, **structured 레인에는 정책 검사가 0건**입니다. 새 목적지가 아니라 **도달 경로**의 문제이므로 신규 스위치 없이 기존 `allowedAgents`를 새 초크포인트에 걸어 **같은 릴리스에서 닫았습니다** — attach funnel과 codex 스폰 resolver 두 곳이 강제하고, 가용성 프로브와 렌더러 좁히기가 진입점을 미리 없앱니다. `allowedAgents`의 네 번째 축이며 상세는 `docs/reference/enterprise-policy.md`의 해당 행 ④에 있습니다.

> **v1.4.194 갱신 (2026-09-02).** 이 판에서 재검증한 것은 **v1.4.193 → v1.4.194 구간의 델타**입니다(트리 diff 1,075 파일, +90,529/−53,536, 신규 491건 · 삭제 5건 · 이름변경 8건). 판정은 양 태그의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)(`/`new WebSocket`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 렌더러 호출지점 대조 + 스킴 URL 호스트 집합 차집합(`src`·`config`·`resources` 전체) + `openExternal`·IPC 채널·릴레이 RPC 메서드·`GlobalSettings` 키·CLI 커맨드·`program:` 스폰 리터럴·의존성·`.github/` 차집합 + **신규/이동 소스 파일 425건 전수 네트워크 프리미티브 스캔**(`axios`/`undici`/`got`/`node-fetch`/`sendBeacon`/`tls.connect`/`net.connect`/`dgram`/`curl`/`wget` 포함)입니다.
>
> **신규 외부 목적지 0건.** 프로덕션 호출지점 **56 → 56**(집합 동일; 유일한 차이는 `git fetch` 래퍼 메서드가 `git-handler.ts`에서 `git-handler-fetch-operations.ts`로 **이사한 것**뿐입니다), 렌더러 **5 → 5** 집합 동일, `openExternal` 107 → 108(증가분은 파일 분해로 생긴 호출지점 이동), 하드코딩 호스트 차집합은 전부 테스트 픽스처(`proxy.example` 자격증명 문자열)입니다. 신규 IPC 채널 5건(`orcaProfiles:list`/`authStatus`, `pet:importPetBundle`, `workspaceCleanup:getCachedScan`, `workspaceSpace:getCachedAnalysis`)은 모두 **로컬 전용**입니다 — `orca-profiles.ts`에 네트워크 호출이 0건이고, `pet:importPetBundle`은 `dialog.showOpenDialog` 로컬 파일 읽기, 나머지 둘은 기존 스캔의 캐시 조회 변종입니다. 릴레이 RPC 메서드·`GlobalSettings` 키·CLI 커맨드·`program:` 스폰 리터럴은 **차집합 0**, 신규 의존성은 `@tiptap/extension-code-block`·`@tiptap/extension-list`(에디터, 네트워크 없음)뿐입니다. `.github/`의 유일한 신규 항목은 `uses: pnpm/setup@`(pnpm 12 전환)입니다. 신규 도메인 4곳(`main/claude-accounts/runtime-auth`, `main/runtime/runtime-rpc`, `renderer/components/gitlab-item-dialog`, `renderer/lib/typing-latency`)은 전부 기존 파일의 분해이며 네트워크 프리미티브가 0건입니다 — `typing-latency`는 `console.log`로만 보고합니다. 신규 파일 전수 스캔이 잡은 `curl`은 `codex-hook-script.ts` 한 건인데 목적지가 `http://127.0.0.1:${ORCA_AGENT_HOOK_PORT}` + `--noproxy 127.0.0.1`(루프백)이고, 그 명령 빌더(`agent-hooks/hook-post-command.ts`)는 이 구간에 **한 줄도 바뀌지 않았습니다**.
>
> 🟡 **egress는 아니지만 이번 판에서 고친 것 하나 — 프록시 준비 게이트와 허용목록이 같은 리스너 슬롯을 다툽니다.** v1.4.194가 `session.defaultSession`에 `installElectronProxyRequestGuard`를 등록하기 시작했는데, Electron은 세션당 `webRequest.onBeforeRequest` 리스너를 **하나만** 유지합니다. 포크의 `installEnterpriseNetworkGuard()`는 시작 순서상 **더 뒤에** 등록되므로 `enforceNetworkAllowlist`가 켜진 배포에서 업스트림의 프록시 전환 홀드를 **조용히 대체**하게 됩니다(허용목록 자체는 살아 있으므로 보안 통제는 유지되지만, 프록시 전환 도중 요청이 옛 프록시로 나갈 수 있습니다). 한 리스너 안에서 **허용목록 먼저, 준비 게이트 나중**으로 체이닝해 해소했고 뮤테이션 검증까지 마쳤습니다(`src/main/enterprise/enterprise-network-guard.ts`, 동작 테스트 `enterprise-network-guard.test.ts`의 "still honors the proxy readiness gate it shares the listener slot with"). **다음 동기화에서도 이 슬롯은 계속 다툼 대상입니다** — 업스트림이 `defaultSession`에 새 `onBeforeRequest`를 하나만 더 넣어도 같은 일이 재발합니다.
>
> ✅ **v1.4.193의 §0.2 #31(structured 에이전트 세션 레인)은 이 구간에서도 닫힌 채입니다.** `agentSession.*` RPC는 19 → 19로 변동이 없고, `assertAgentAllowedByEnterprisePolicy` 강제는 10 → **15곳**으로 늘어난 상태가 유지됩니다(`codex-structured-launch-resolution.ts`의 스폰 직전 게이트 포함).

> **v1.4.195 갱신 (2026-09-03).** 이 판에서 재검증한 것은 **v1.4.194 → v1.4.195 구간의 델타**입니다(트리 diff 1,480 파일, +102,149/−28,057, 신규 511건 · 삭제 7건 · 이름변경 26건). 판정은 양 태그의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)`/`new WebSocket`/`axios`/`undici`/`node-fetch`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 렌더러 호출지점 대조 + 스킴 URL 호스트 집합 차집합(`src`·`config`·`resources`) + `openExternal`·IPC 채널·RPC 메서드·CLI 커맨드·`GlobalSettings` 키·`TuiAgent` 유니온·spawn 리터럴·의존성·`.github/` 차집합 + **신규 소스 파일 267건 전수 네트워크 프리미티브 스캔** + **덮어쓰기형 세션 API 등록지점 전수 열거**입니다. IPC 채널 집계는 이번 판부터 **멀티라인 인식 스캐너**로 합니다 — `git grep`의 라인 단위 매칭은 `ipcMain.handle(\n  '채널명',` 형태를 놓쳐 383건으로 과소 계상했고, 실제는 743 → 731건입니다.
>
> **신규 외부 목적지 0건.** 프로덕션 호출지점 **75 → 75**(파일 52개까지 **집합 동일**; 유일한 차이는 테스트용 WebSocket 서버가 `remote-runtime-shared-control-connection.test.ts`에서 `src/shared/remote-runtime-shared-control-test-server.ts`로 **분리된 것**뿐입니다), 렌더러 **7 → 7** 집합 동일, `openExternal` **108 → 108** 집합 동일, 신규 npm 패키지 **0개**(버전 범프 10건 — xterm 9종과 `zod ~4.4.3→~4.5.4`), CLI 커맨드 신규 2건은 둘 다 기존 목적지(`orca open-url`은 v1.4.194의 `orca tab create --url`과 같은 `browserTabCreate`, `orchestration request-show`는 로컬 DB 읽기)입니다. 호스트 문자열 차집합에서 잡힌 6건은 전부 오탐입니다 — `jira.example`/`redirect.test`는 테스트·placeholder 문자열이고, `stackoverflow.com`·`html.spec.whatwg.org`·`www.unicode.org`·`www.w3.org`는 **xterm 패치 파일이 beta.286/287 → beta.299/300으로 갈리며 벤더 번들 텍스트가 바뀐 부수효과**입니다(같은 이유로 v1.4.194에 있던 `bellard.org`가 사라졌습니다). 신규 spawn 리터럴 4건(`/bin/{ln,mv,ps,rm}`)은 `src/main/codex-accounts/`의 **테스트 하네스 전용**이며 래칫 허용목록에 정식 등재됐고, 같은 diff에서 프로덕션 파일 하나가 허용목록에서 빠져 **프로덕션 기준 −1**입니다. 신규 도메인 `src/main/codex-accounts/`(14건)는 WSL 안의 레거시 codex 인증 파일을 옮기고 지우는 **로컬 sh/mv/rm 실행기**로, `curl|wget|http|nc|ssh|scp|fetch` 매치가 0건입니다 — 자격증명을 만지지만 밖으로 내보내지 않습니다.
>
> ✅ **제품이 아니라 저장소·CI에 새 표면이 하나 들어왔고, 지웠습니다 — `docs/site/` + `.github/workflows/docs.yml`.** 업스트림이 Next.js 문서 사이트(신규 115파일, 자체 `package.json`·`pnpm-lock.yaml`)와 그것을 **Vercel `www.onorca.dev/docs`로 배포하는 워크플로**를 들여왔습니다. **들어온 상태에서도 제품 위험은 0이었습니다** — 데스크톱 번들 제외(`config/electron-builder.config.cjs:152`의 `'!docs{,/**/*}'`, `src/`에서 참조 0건), 루트 워크스페이스 제외(`pnpm-workspace.yaml`의 `packages: []`, 루트 lockfile에 `fumadocs|vercel@|next@16` 매치 0건), 배포 잡은 `production`이 `needs: release_gate`이고 그 `release_gate`가 `github.repository == 'stablyai/orca'`로 막혀 **이 포크에서 도달 불가**였습니다. 그럼에도 **저장소에 벤더 인프라를 향하는 배포 파이프라인과 별도 의존성 트리(next·fumadocs·`vercel` CLI)를 남겨 둘 이유가 없어 둘 다 제거했습니다.** 같이 지운 것: `release-cut.yml`의 `docs-production-dispatch` 잡(`gh workflow run docs.yml` 호출), `config/scripts/release-cut-token-permissions.test.mjs`의 `EXPECTED_MATRIX` 4행, `.gitignore`의 `!docs/site/` 예외 2행. **다음 동기화마다 DU 충돌로 돌아오며, `release-cut.yml`과 토큰 매트릭스는 충돌 없이 조용히 되살아납니다** — 재발 절차는 `README.md` §6의 「디렉터리 통째로 지운 것 — `docs/site`」에 적어 두었습니다. 신규 서드파티 액션 `pnpm/action-setup@v5`도 이 제거로 함께 사라졌습니다.
>
> ✅ **게이트 유실 0건 / 신규 우회로 0건.** 26개 정책 스위치 전부 소비자 파일수·라인수가 포크 tip과 동일하고, 게이트 라인 문자열 대조도 594 → 595(늘어난 1줄은 `max-lines`로 추출한 포크 모듈의 import)입니다. `src/main/enterprise/` 16개 파일과 `src/shared/enterprise-policy.ts`는 포크 부모 ↔ HEAD **바이트 동일** — 머지가 가드를 한 글자도 건드리지 않았습니다. 머지가 추가한 외부 도달 표면은 RPC 3건(`browser.openUrl`·`jira.searchUsers`·`orchestration.requestShow`)·IPC 2건(`jira:searchUsers`·`runtimeEnvironments:retryControlConnection`)뿐이고 **어느 것도 `assertAgentAllowedByEnterprisePolicy` 아래로 흐르지 않습니다**(스폰 프리미티브 미도달). `agentHooks:*` IPC 14건은 오히려 **삭제**돼 표면이 줄었습니다(런타임 RPC로 이동). `TuiAgent` 유니온과 `GlobalSettings` 키는 바이트 동일이라 `allowedAgents`에 등재할 신규 에이전트가 없습니다.
>
> ✅ **단일 리스너 슬롯(v1.4.194 판의 위험) 경합 0건.** `onBeforeRequest` 등록지점 4곳 중 `defaultSession`을 쓰는 것은 업스트림 프록시 게이트(`startup/main-process-ready-foundation.ts:242`, `:246`)와 포크 허용목록(`startup/main-process-ready-foundation.ts:252`) 한 쌍뿐이고, 같은 `app.whenReady()` 본문 안에서 **포크가 마지막에 등록해 이깁니다**. 포크 리스너가 업스트림 로직을 자기 안에서 체이닝하므로(`enterprise-network-guard.ts:95-105`) 프록시 준비 게이트도 살아 있습니다. 나머지 2곳은 `persist:` 파티션입니다. `onHeadersReceived`는 저장소 전체 0건, `app.configureHostResolver`는 포크의 `enterprise-secure-dns.ts:24` 유일, 등록지점 델타는 v1.4.194→195과 포크부모→HEAD 모두 **0**입니다.
>
> ✅ **방어심도 관찰 1건 — 같은 릴리스에서 닫았습니다.** 신규 IPC `runtimeEnvironments:retryControlConnection`(`src/main/ipc/runtime-environment-connectivity-handlers.ts`)에 `assertRemoteOrcaServerAllowed()`가 없었습니다 — 같은 파일의 형제 핸들러 2곳은 들고 있어 패턴이 비대칭이었습니다. **발견 시점에도 무해했습니다**: 본문이 `retryRemoteRuntimeSharedControlConnectionNow()` 하나뿐이고 그 구현은 `sharedControlConnections.get(id)?.connection.retryNow()`(`runtime-environment-request-connections.ts:115-117`)라 **캐시에 이미 있는 연결만** 재시도하는데, 그 캐시를 채우는 `getSharedControlConnection()`의 호출지점 3곳이 전부 `disableRemoteOrcaServer` 게이트 뒤에 있습니다(`transport-routing.ts:68`·`:144`·`:252`). **그래도 닫은 이유**: 안전성이 자기 게이트가 아니라 "캐시는 게이트 뒤에서만 채워진다"는 **업스트림 소유 불변식**에 의존하므로, 업스트림이 게이트 밖에서 shared-control 연결을 만드는 순간 게이트 없는 재연결 레인이 됩니다. 핸들러에 한 줄을 넣고 동작 테스트 `src/main/ipc/remote-orca-server-retry-control-connection-policy.test.ts`로 고정했으며, **게이트를 임시로 지워 그 케이스만 빨개지는 것까지 확인했습니다(뮤테이션 검증).** 같은 형태의 무게이트 채널 `runtimeEnvironments:retryConnectionsNow`(복수형, v1.4.194부터 존재)는 같은 불변식에 의존하며 이번에는 손대지 않았습니다 — 다음 판의 후보입니다.

> **v1.4.196 갱신 (2026-09-04).** 이 판에서 재검증한 것은 **v1.4.195 → v1.4.196 구간의 델타**입니다(트리 diff 2,326 파일, +290,738/−202,948, 신규 1,553건 · 삭제 1건 · 이름변경 6건). 판정은 양 태그의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)(`/`new WebSocket`/`axios`/`undici`/`node-fetch`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 스킴 URL 호스트 집합 차집합(`src`·`config`·`resources`) + 추가 라인 전수 네트워크 프리미티브 스캔 + `openExternal`·IPC 채널(멀티라인 인식)·RPC 메서드·의존성 차집합입니다.
>
> ✅ **신규 외부 목적지 0건.** 프로덕션 네트워크 호출지점은 **236 → 236으로 동수**이고, 추가 라인 전체에서 `net.fetch`/`await fetch(`/`https?.request(`/`new WebSocket` **신규 호출지점 0건**입니다. `shell.openExternal` 호출지점도 10 → 10. 호스트 리터럴 차집합에 뜬 4건(`acme.dev`, `docs.legit.dev`, `readme.io`, `unrelated.example`)은 전부 **테스트 픽스처이거나 주석**입니다 — 유일하게 프로덕션 파일에 있는 `readme.io`는 `tab-create-entry-network-selection.ts:35`의 설명 주석이고 코드가 아닙니다. IPC 채널은 688 → 682로 **6건 감소**했고, 신규 11건은 전부 로컬 용도(`fs:authorizeExternalPath`, `git:cancelStatus`, `browser:tab*Reply`, `app:*Startup*`, `ui:consumePending*`)입니다.
>
> ⚠️ **이 릴리스의 위험은 신규 목적지가 아니라 게이트 유실입니다.** upstream이 `max-lines` 우회 34건을 한 릴리스에서 청산하면서(§README 6절) 이 포크의 게이트가 박혀 있던 파일이 거의 전부 해체됐습니다 — `src/main/index.ts` 3,805 → 65줄, `orca-runtime.ts` −44,399줄, `preload/index.ts` −5,302줄, `rate-limits/service.ts` −2,180줄, `browser/browser-manager.ts` −2,590줄, `store/slices/{ui,tabs}.ts` −5,025줄. **게이트를 옮기지 않으면 코드가 통째로 다른 파일로 가면서 게이트만 사라지고, 타입체크도 테스트도 그것을 잡지 않습니다.** 이번 판은 정책 호출지점 **62 → 62**, 포크 전용 모듈 호출지점 **706 → 715**로 대조해 유실 0건을 확인했고, 각 게이트의 새 위치는 머지 커밋 본문에 열거했습니다.
>
> 🔴 **되살아난 표면 31건 — 충돌 없이 들어왔습니다.** 포크가 지운 기능을 upstream이 **새 경로에 새 파일로** 다시 만들면 git은 충돌을 내지 않습니다: 인앱 자동 업데이트(`src/main/updater.ts` → `src/main/updater/**` 16개 + `updater-test-module-loader` 2개), preload 브리지 3종(`api/{updater,feedback,bitbucket}-bridge.ts`), 문서 사이트 자산 8건(`docs/site/**`). 전부 다시 제거했습니다. **다음 동기화에서도 같은 형태로 재유입될 수 있으므로**, 트리 diff의 신규 파일 목록을 포크 삭제 목록(381건)의 도메인 키워드로 훑는 절차를 유지하십시오.
>
> ✅ **스킬 패키지 다운로드 레인 재확인.** 런타임 분할로 신설된 `src/main/runtime/runtime-skill-install-commands.ts`가 `https://storage.googleapis.com`을 허용 오리진으로 들고 있지만, 실제 다운로드는 `executeSkill{,Bundle}InstallRequest` → `downloadSkillPackageGrant`로 흐르고 그 함수 안의 `SKILL_SHARING_REMOVED` 초크포인트(`skill-package-download.ts:250`)는 그대로입니다. 동작 테스트 `src/main/skills/skill-sharing-removed.test.ts` 7건 통과로 확인했습니다.
>
> ✅ **egress 재점검 (같은 판, 2차).** 1차 판정 뒤 외부 URL 접근 로직만 다시 훑었습니다. **프로덕션 네트워크 프리미티브를 소유한 파일은 39 → 39로 집합까지 동일**하고(호출지점이 새 모듈로 이동한 것이 0건), 그중 **32개는 머지 전 포크와 바이트 동일**합니다. 유일하게 바뀐 `ipc/worktree-remote.ts`의 변경은 전부 로컬 git·표시이름 리팩터이고 그 파일의 `fetch(` 2건은 HTTP가 아니라 git fetch 클로저입니다(오탐). **초크포인트 78개 중 바뀐 것은 8개뿐이고 전부 이번에 의도적으로 이관한 것들**입니다(`browser-manager-{bindings,guest-popup-policy}`, `rate-limits/service/service-state`, `startup/main-process-plugins`, `slices/{tabs/tabs-reconciliation,ui/ui-slice-{removed-views,view-actions,hydration-sanitizers}}`) — 나머지 70개는 바이트 동일. `shell.openExternal` 11 / `loadURL` 16 / `setWindowOpenHandler` 7로 수도 동일합니다.
>
> 🔴 **1차 판정이 틀렸던 것 — 테스트를 저장소 설정 없이 돌렸습니다.** `vitest run`을 `--config config/vitest.config.ts` 없이 실행하면 별칭(`@/`)·setup 파일·include 패턴이 전부 빠져 **정책 게이트 테스트가 아예 실행되지 않고**(모듈 해석 실패로 수집 단계에서 죽음) `out/` 스테일 산출물까지 수집되어 실패가 3,800건대로 부풀려집니다. 올바른 설정으로 재실행한 결과는 **69,021 테스트 중 실패 4건(1파일)**이고, 그 1파일(`codex-fetcher-pty-settle.test.ts`)은 머지 전 포크와 순정 v1.4.196 양쪽에서도 실패하는 기존 문제입니다. **정책 게이트 45개 파일 343개 단언 전부 통과** — 이번에야 실제 동작 증명이 생겼습니다. 그 과정에서 1차 판정이 놓친 회귀 2건도 잡았습니다: 삭제한 updater 모듈에 함께 들어 있던 UI 액션 6종, 그리고 업스트림이 모순인 채로 출하한 Windows 서명 계약 테스트.
>
> ✅ **`file:line` 인용 8건 갱신.** 이관한 게이트의 인용을 실제 위치로 고치고 각 줄이 해석되는지 확인했습니다 — 플러그인 RPC 게이트(`startup/main-process-plugins.ts:120`), 플러그인 IPC 게이트(`register-core-handlers/register-core-handlers.ts:205`), 사용량 폴링(`rate-limits/service/service-state.ts:32`)과 그 칩 정책(`service/service-result-policy.ts:99`), 맞춤법(`browser/browser-manager-guest-popup-policy.ts:30`), 네트워크 가드 등록(`startup/main-process-ready-foundation.ts:252`), 그리고 **잔여 위험 #20-④의 게이트 없는 세 번째 `shell.openExternal`**(`browser-manager.ts:749` → `browser-manager-guest-popup-policy.ts:190`, 코드는 그대로 이동만 했습니다 — 여전히 정책을 보지 않고, 닫으려면 여전히 1줄이며 그 파일은 이제 `getEnterprisePolicy`를 import 합니다).
>
> ⚠️ **이 구간의 판정도 정적 분석입니다.** 코드 경로로 판정했고 패킷 캡처는 하지 않았습니다. **위 8건 외의 `file:line` 인용은 이 판에서 전면 재검증하지 못했습니다** — 해체 규모상 이 문서와 README의 기존 인용 상당수가 밀려 있습니다(예: §0.2 #19의 `src/main/startup/main-process-plugins.ts:120`, `register-core-handlers.ts:201`, #20의 `browser-manager.ts:744-788`). 인용을 따라가기 전에 심볼로 다시 grep하십시오.

> **v1.4.197 갱신 (2026-09-04).** 이 판에서 재검증한 것은 **v1.4.196 → v1.4.197 구간의 델타**입니다(트리 diff 2,827 파일, +228,776/−42,350, 신규 966건 · 삭제 140건 · 이름변경 82건). 판정은 머지 전 포크 tip ↔ 머지 결과의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)(`/`new WebSocket`/`axios`/`undici`/`node-fetch`/`sendBeacon`/`tls.connect`/`net.connect`/`EventSource`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 스킴 URL 호스트 집합 차집합(`src`·`config`·`resources`) + 추가 라인 전수 네트워크 프리미티브 스캔(206,735줄) + `openExternal`·`loadURL`·`setWindowOpenHandler`·IPC 채널(멀티라인 인식)·의존성·`.github/` 차집합입니다.
>
> ✅ **신규 외부 목적지 0건.** 프로덕션 네트워크 프리미티브를 소유한 파일이 **48 → 48로 집합 동일**하고 호출지점도 **70 → 70**입니다(호출지점이 새 모듈로 이동한 것조차 0건). `shell.openExternal` 13 → 13, `loadURL` 23 → 23, `setWindowOpenHandler` 150 → 150, npm 의존성 추가·제거 **0건**. 호스트 리터럴 차집합의 신규 8건은 전부 테스트 파일입니다(`assistant.ai`·`example.museum`·`m4air.example.com`·`nested.example.com`·`openclaw.example.com`·`push.example.invalid`, 그리고 `https://pr/7`·`https://mr/7`에서 잘려 나온 `pr`·`mr`). IPC 채널은 **696 → 698**이고 신규 3건은 전부 로컬입니다 — `app:awaitGitEnvironmentStartupBarrier`(시작 배리어), `clipboard:readImageThumbnail`(로컬 클립보드), `ui:consumePendingMarkdownFileOpens`(OS "Open With" 큐 배출). `workspaceCleanup:hasKillableLocalProcesses` 1건은 삭제됐습니다.
>
> ✅ **신규 텔레메트리 이벤트 2건 — 기존 초크포인트 아래입니다.** `daemon_adopted`·`daemon_pty_cwd_denied`(`src/main/daemon/daemon-adoption-telemetry-event.ts`, 신규 파일)가 들어왔습니다. 둘 다 `src/main/telemetry/client.ts`의 `track()`을 거치고, `track()`은 `resolveConsent()`를 통과해야 하며 거기 **정책 파일의 `disableTelemetry` 킬스위치**가 그대로 있습니다(`src/main/telemetry/consent.ts:88`). 렌더러 쪽 신규 이미터(`native-chat-telemetry`, `use-feature-wall-tour-telemetry`)도 `window.api.telemetryTrack` → 같은 main 초크포인트로 흐릅니다. 덧붙여 텔레메트리 키는 여전히 컴파일 타임 상수라 사내 빌드는 키 없이 나갑니다.
>
> 🔴 **저장소에 새 최상위 표면이 하나 들어왔고, 지웠습니다 — `cloud/` + `cloud-*` 워크플로 25개.** upstream이 모바일 앱↔데스크톱 페어링 **릴레이 서비스의 GCP 인프라 전체**를 모노레포에 들여왔습니다(신규 331파일 4MB: 자체 pnpm 워크스페이스 `apps/relay`·`apps/relay-fence-broker`·`apps/relay-ops`·`packages/relay-contract`, Terraform 루트, `.github/actions/cloud-sql-rollout-lease`). **제품 위험은 들어온 상태에서도 0이었습니다** — 데스크톱 빌드가 `cloud/`를 참조하지 않고, 클라이언트 쪽 레인은 이미 `disableMobilePairing`·`disableCloudRelay`가 막습니다. 그럼에도 지운 이유는 **저장소·CI 표면**입니다: 25개 중 2개가 **스케줄 트리거**(`cloud-monitor-relay-clock-skew.yml` 매시, `cloud-power-relay-staging.yml`)라 사내 origin에서 Actions가 켜져 있으면 벤더 GCP 프로젝트를 향해 매시 인증을 시도하고, `cloud-verify.yml`은 PR 트리거이며, 보안 리뷰어가 사내 저장소에서 벤더 배포 파이프라인을 보게 됩니다. `.github/CODEOWNERS`의 해당 3행도 같이 지웠습니다. **원장에 등재했습니다** — `config/fork-feature-ledger.json`의 `absentPaths`에 `cloud`·`.github/workflows/cloud-*.yml`·`.github/actions/cloud-sql-rollout-lease` 3건. 워크플로는 파일 하나가 아니라 **계열**이므로 게이트에 글롭 지원을 더했고(`config/scripts/fork-feature-ledger.test.mjs`의 `matchesInDirectory`), 더미 `cloud-probe-mutation.yml`을 넣어 빨개지는 것까지 확인했습니다(뮤테이션 검증).
>
> 🔴 **되살아난 표면 3건 + 조용히 사라진 포크 게이트 6건.** 되살아난 것: `src/main/linux-package-downloaded-status.ts`, `src/main/updater.linux-externally-managed.test.ts`, `src/renderer/src/components/settings/GeneralUpdateSettingsSection.test.tsx` — 전부 v1.4.197 **신규 파일**이라 충돌 없이 들어왔고, 지운 updater 레인을 import 합니다. 사라진 게이트: ① `allowedAgents`의 structured-native-chat 소비 지점(upstream이 `structured-native-chat-availability.ts`를 삭제 → `lib/agent-launch-routing.ts`의 `resolveAgentLaunchRoute`로 이관, 뮤테이션 검증 완료), ② `disableStarNag`가 붙어 있던 `checkOrcaStarred`(upstream이 코얼레싱으로 재작성 → 게이트를 새 진입점에 재삽입, 뮤테이션 검증 완료), ③ 서브모듈 SCM의 실행 호스트 라우팅(upstream이 `RuntimeGitTarget.connectionId`를 폐기 → `requireRuntimeGitProvider(target)`로 9곳 이관), ④ 패키징 계약 테스트의 번들 정책 픽스처(파일 분해로 유실 → `electron-builder-runtime-resources.test.mjs`로 이관), ⑤ 탭 팝아웃의 선택적 링크 라우팅 훅, ⑥ 앱 버전 브리지(upstream은 updater 브리지로 주므로 이 빌드는 `app-bridge.ts`가 유일한 공급자). **①②④⑥과 서브모듈 브리지는 이번에 원장에 앵커로 등재했습니다**(74 → 76건).
>
...
>
> **v1.4.198 갱신 (2026-09-08).** 이 판에서 재검증한 것은 **v1.4.197 → v1.4.198 구간의 델타**입니다(트리 diff 2,079 파일, +151,061/−18,809, 신규 663건 · 삭제 21건 · 수정 1,395건). 판정은 머지 전 포크 tip ↔ 머지 결과의 **프로덕션 네트워크 호출지점 집합 대조**(`fetch(`/`net.fetch`/`https?.(get|request)(`/`new WebSocket`/`EventSource`/`axios`/`undici`/`node-fetch`/`sendBeacon`/`tls.connect`/`net.connect`, `src/{main,shared,relay,cli,preload}`, 테스트 제외) + 스킴 URL 호스트 집합 차집합(`src`·`config`·`resources`) + IPC 채널 집합 차집합(`ipcMain.handle|handleOnce|on` + `ipcRenderer.invoke|send|on`, 다중행 인식) + RPC 메서드·`openExternal`/`loadURL`/`setWindowOpenHandler`·의존성 차집합입니다. 나머지 확정 항목은 각자의 기준 시점 서술이며 이번에 다시 열어보지 않았습니다.
>
> ✅ **신규 외부 목적지 0건.** 프로덕션 네트워크 호출지점이 **75 → 75로 라인 문자열까지 집합 동일**합니다(새로 들어온 줄 0건). IPC 채널은 **962 → 962**로 이름 하나 늘지 않았고 — 이 릴리스의 신규 표면은 전부 런타임 RPC 쪽입니다 — 신규 RPC 메서드 6건(`agentSession.requestHandoff`·`agentSession.reveal`·`agentSession.subscribeStatus`·`markdown.readTab`·`markdown.saveTab`·`runtime.clientCapabilities.update`)은 모두 로컬 동작입니다. 스킴 URL 호스트 집합에 11개가 늘었으나 **목적지는 하나도 없습니다**: 테스트 픽스처(`*.example.test`·`gw.test`·`*.invalid`·`mirror`·`settings.example.test`), 파비콘 테스트 상수(`github.githubassets.com`), 포크 자신이 "여기에 접속하지 않는다"고 적어 둔 주석(`registry.npmjs.org` — `src/cli/handlers/skills-offline-install.ts:91`), 그리고 upstream 이 새로 쓴 `@vscode/windows-process-tree` 패치 본문의 MSDN 문서 주석(`msdn.microsoft.com`)입니다. `openExternal`/`loadURL`/`setWindowOpenHandler` 집합 증가분 1건은 `.electron.test.ts` 안입니다.
>
> 🆕 **이 구간에서 실제로 바뀐 것은 의존성 하나입니다 — `@anthropic-ai/claude-agent-sdk@0.3.251`**(제거 0건). structured 네이티브 챗 레인에 **두 번째 제공자(Claude)** 가 생겼고, 그 stdio 프로토콜 드라이버입니다. **Orca 가 자식 프로세스를 직접 만들어 SDK 에 건네므로**(`spawnClaudeCodeProcess` → 창구 `spawnProcess`, `src/main/claude/claude-agent-sdk-process-spawn.ts`) 목적지는 **사용자 자신의 `claude` CLI** 이고, SDK 가 동봉하는 플랫폼별 CLI 바이너리 8종(플랫폼당 ~95MB)은 `pnpm-workspace.yaml` 의 `ignoredOptionalDependencies` 로 설치에서 제외됩니다(실측: `node_modules/@anthropic-ai/` 에 CLI 바이너리 없음). 이 레인의 egress 는 **#1 의 자식 프로세스 사각지대와 같은 자리**입니다 — 새로 생긴 것이 아니라 `codex app-server` 옆에 provider 가 하나 더 선 것입니다(#31·#32).
>
> 🔒 **포크가 이 구간에서 닫은 것 1건**: structured 레인의 **마지막 스폰 게이트가 Claude 쪽에는 없었습니다.** attach 퍼널(`agentBlockedByPolicyError`)과 가용성 프로브·렌더러 좁히기는 provider-agnostic 이라 진입은 이미 막았지만, **이미 저장된 레코드를 wake·소유자 교체·재시작 복원에서 다시 띄우는 경로는 attach 를 두 번 지나지 않습니다** — codex 쌍둥이(`codex-structured-launch-resolution.ts:48`)에만 있던 refusal 입니다. `claude-structured-launch-resolution.ts` 에 같은 게이트를 넣고 행동 테스트 2건(거부/무정책 통과)과 뮤테이션 검증(게이트를 지우면 자기 케이스만 빨개짐)을 붙였으며, 원장에 두 줄 다 앵커로 등재했습니다(#32).
>
> ⚠️ **미검증으로 남기는 것 — SDK 번들 안의 벤더 엔드포인트.** `sdk.mjs`(1.4MB, 미니파이)는 그 레인이 처음 뜰 때 **Electron 메인 프로세스로 동적 import** 되고, 번들 안에 `api.anthropic.com`·`mcp-proxy.anthropic.com`·`beacon.claude-ai.staging.ant.dev` 등 호스트 리터럴이 들어 있습니다. 실제 네트워크 호출지점은 4곳입니다 — ① ② `${baseURL}` 에 대한 OAuth/OIDC 토큰 교환 2건, ③ ④ 원격 세션 서버(`${serverUrl}/sessions`, 응답에 `ws_url`) 2건. **Orca 는 이 넷 중 어느 것도 지나가지 않습니다**(auth provider 도 `serverUrl` 도 넘기지 않고 transport 는 stdio 입니다). 통제 수단도 남아 있습니다 — **넷 다 전역 `fetch` 를 쓰므로 `enforceNetworkAllowlist` 의 fetch 래퍼가 볼 수 있고**(`undici`·`node:https`·`https.request` 사용 0건), 이는 #1 의 자식 프로세스 사각지대와 반대입니다. 🔴 **다만 `new WebSocket` 1건(원격 세션 브리지)은 fetch 래퍼도 세션 가드도 보지 못합니다.** 이 판정은 미니파이 번들에 대한 **정적 추론**이지 패킷 캡처가 아닙니다.
>
> 🚫 **되살아났다가 다시 지운 것.** upstream 이 `cloud/**` 14파일과 `.github/workflows/cloud-*.yml` 4건을 다시 들여왔고 원장의 `absentPaths` 가 먼저 잡았습니다 — `git rm` 했습니다(README §6 절차 그대로). `docs/site` 2건도 같습니다. 한편 **v1.4.198 이 크게 확장한 `src/main/runtime/relay/**` 는 이 빌드에서 도달 불가입니다** — 리전 프로브(`${origin}/health`), 디렉터 조회(`${directorUrl}/v1/regions`), 컨트롤 WebSocket(`ws` 패키지) 모두 `DesktopRelayService` 안에 있고, 그 서비스는 `getOrcaCloudAuthConfig()` 가 configured 를 돌려줄 때만 생성되는데 `ORCA_CLOUD_REMOVED` 가 **정책 검사보다 먼저** 그것을 막습니다(§3.1). 호출지점 수가 늘지 않은 이유가 이것입니다: 파일들은 이전부터 있었고 이번에 내용만 커졌습니다.
>
> ➕ **부수 변화 2건.** ① 신규 로케일 `fr.json` — 네트워크와 무관하며, 포크가 지운 표면(스킬 공유 등)에 대한 잔여 항목 103건을 정리했습니다. ② upstream 이 `@vscode/windows-process-tree` 패치를 **EDR 자세 개선**으로 다시 썼습니다 — 명령줄을 PEB 에서 읽던 것을 커널의 `ProcessCommandLineInformation` 로 바꾸고 `PROCESS_QUERY_INFORMATION | PROCESS_VM_READ` 를 `PROCESS_QUERY_LIMITED_INFORMATION` 로 낮췄습니다. **egress 중립이고 자격증명 덤핑 프리미티브를 줄이는 방향**입니다. 포크의 Spectre 요구사항 제거는 그 위에 그대로 얹혀 있습니다. §8 "종결: 로케일 카탈로그의 Google Translate" 항목의 인용은 upstream 이 `bootstrap-zh-catalog.mjs` 를 일반화하면서 **`bootstrap-locale-catalog.mjs:71`** 로 옮겼습니다 — 여전히 빌드 스크립트 한정이고 `package.json` 스크립트에도 CI 워크플로에도 물려 있지 않습니다.
>
...
| `disableSpellcheck` | boolean | = `lockdown` | `webPreferences.spellcheck`를 끄는 지점 **6곳**: 메인 창(`src/main/window/createMainWindow.ts:306`), `will-attach-webview` 게스트(`:494`), 대시보드 팝아웃 창(`src/main/window/dashboard-popout-window.ts:181`), 분리된 탭 창(`src/main/window/tab-popout-window-factory.ts:49`), 오프스크린 브라우저 백엔드(`src/main/browser/offscreen-browser-backend.ts:45`), PDF 내보내기 WebContents(`src/main/lib/html-to-pdf.ts:46`) |
...
| 30 | **허용목록 밖 에이전트의 managed hook 런처** (egress가 아니라 **로컬 잔여 상태** 항목입니다) | — (전부 로컬 파일. 런처 자신은 소켓을 열지 않고, 훅 이벤트는 `127.0.0.1`의 Orca 훅 서버로만 POST 합니다) | 앱이 뜰 때·런타임 재조정·설정에서 에이전트 상태 훅을 켤 때. 예전 빌드는 이 경로에서 `allowedAgents`를 전혀 보지 않았으므로, **PATH에 CLI가 감지되기만 하면** 허용목록 밖 에이전트에도 런처가 깔렸습니다 | ✅ **v1.4.188 신설 — `allowedAgents`의 세 번째 축이 이 표면을 닫습니다.** 목록 밖 에이전트는 훅 런처를 **새로 받지 않고**(설치 목록에서 걸러지며 `skipReason: 'agent_blocked_by_policy'`로 보고됩니다), 이미 깔린 것은 다음 실행에 지워집니다 — 각 에이전트 설정 파일(`~/.gemini/settings.json`, `~/.cursor/hooks.json`, `~/.factory/settings.json` 등)의 Orca managed 항목을 **먼저** 지우고, 그다음 `~/.orca/agent-hooks/<agent>-hook.{sh,cmd,ps1}`와 같은 폴더의 `<agent>-` 접두 파일을 지웁니다(순서를 뒤집으면 설정이 없는 스크립트를 계속 불러 훅 이벤트마다 에러가 납니다). 정리는 **실제로 설치돼 있을 때만** 돕니다 — 그러지 않으면 설치된 적 없는 벤더 설정 디렉터리를 되레 만들어 냅니다. `allowedAgents`가 `null`이면 전체가 no-op이라 업스트림 빌드·`pnpm dev`는 영향이 없습니다. 🔴 **잔여 1**: **원격/SSH·WSL 호스트는 새 설치만 막고, 이미 깔린 원격 런처는 지우지 않습니다** — 원격 파일 스윕은 구현돼 있지 않으므로 정책 배포 전에 원격 호스트에 깔린 훅은 그 호스트에서 직접 지워야 합니다. 🔴 **잔여 2**: 데스크톱 앱이 **꺼진 상태**에서 `orca agent hooks enable`을 직접 실행하면 게이트를 지나지 않습니다(오프라인 분기). 앱이 떠 있으면 런타임 RPC로 가서 게이트를 지나고, 오프라인으로 깔린 것은 다음 앱 실행의 스윕이 지웁니다 | 초크포인트 `src/main/agent-hooks/enterprise-agent-hook-policy.ts:21`, 스윕 `enterprise-agent-hook-sweep.ts:100`(설정 정리 `:75` → 런처 삭제 `:88`, 설치 여부 확인 `:62`, no-op `:101`), 설치 게이트 `managed-agent-hook-controls.ts:31`·`:144`·`:156`, 원격 `remote-managed-hook-installers.ts:99`, 오프라인 CLI `src/cli/handlers/agent-hooks.ts:199`. 테스트 `managed-agent-hook-policy-gate.test.ts`, `enterprise-agent-hook-sweep.test.ts`, `enterprise-agent-hook-sweep-side-effects.test.ts` |
| 31 | **structured 에이전트 세션 → `codex app-server` 자식 프로세스** (egress가 아니라 **허용목록 우회 경로** 항목입니다) | codex CLI가 정하는 목적지(OpenAI). Orca는 목적지를 모릅니다 — #1과 같은 자식 프로세스 레인 | `agentSession.create` / `agentSession.ensure` RPC 호출 시. 렌더러에서는 `experimentalStructuredNativeChat` + `openAgentTabsInChatByDefault` 두 설정이 켜져야 하고 win32는 `canUseStructuredNativeChat`이 거부하지만, **그 판정은 렌더러에만 있었습니다** — `agent-session.structured.v1`을 광고하는 원격/모바일 클라이언트는 렌더러를 지나지 않고 이 RPC에 바로 닿습니다(⚠️ 정직하게: `orca` CLI와 `mobile/`에는 이 릴리스 기준 `agentSession` 진입점이 **아직 없습니다** — 표면은 열려 있고 사내 클라이언트가 아직 안 쓸 뿐입니다). 또 메인 쪽 지원 판정(`supportsCodexStructuredLocation`)은 로컬·비WSL이면 Windows도 통과시킵니다(프로세스 생성시각을 읽을 수 있을 때 — 이 포크는 패치된 `@vscode/windows-process-tree`를 출하하므로 사내 Windows 플릿에서 통과할 수 있습니다) | ✅ **v1.4.193에서 열렸고 같은 릴리스에서 닫았습니다.** 사내 정책은 `allowedAgents: ["claude", "opencode"]`이고 빌드 내장 바닥값도 같으므로 **codex는 허용되지 않습니다**(`resources/enterprise-policy.json:53`, `src/shared/enterprise-policy-baseline.ts:60`). 그런데 이 포크의 `assertAgentAllowedByEnterprisePolicy` 호출 10곳은 전부 **PTY 스폰**과 **구(舊) 에이전트 세션 RPC**(`ensureAgentSession`/`createAgentSession`)에 있고, structured 레인은 PTY가 아니라 `spawnProcess` JSONL stdio로 뜨므로 그 어느 것도 지나지 않습니다 — structured 표면 전체에서 정책 개념 검색이 **0건**입니다. upstream이 새로 넣은 `structured-agent-session-gate.ts`는 **클라이언트 capability 협상**(`agent-session.structured.v1` 광고 여부)만 보며 관리자 정책과 무관합니다. `enforceNetworkAllowlist`로도 구제되지 않습니다(#1과 동일한 자식 프로세스 사각지대). **신규 스위치는 불필요합니다** — 기존 `allowedAgents`가 정확히 이 의미("어떤 CLI를 띄울 수 있는가")를 가지므로, 초크포인트에 걸기만 하면 됩니다. 후보: ① RPC 진입점 `structured-agent-session-gate.ts`(거부가 RPC 오류로 표면화됨), ② 스폰 직전 `codex-structured-launch-resolution.ts`의 resolver(진입점과 무관하게 막힘 — 진짜 초크포인트). **둘 다 걸었고 뮤테이션 검증까지 마쳤습니다** — 네 게이트를 각각 하나씩만 임시 제거해 자기 케이스만 빨개지는 것을 확인했습니다(attach 2건 / 스폰 1건 / 프로브 1건 / 렌더러 1건). 여기에 더해 두 곳이 거부를 미리 알립니다: `getStructuredAgentSessionCreateSupport`는 기존 값 `reason: 'agent'`로 `{supported:false}`를 돌려주고(신규 wire 값 없음) 차단된 에이전트가 structured 호스트를 존재하게 만드는 것도 막습니다 — 다만 **이 프로브는 현재 저장소 안에 호출자가 없습니다**(원격/모바일용 표면). 데스크톱 렌더러는 프로브 없이 `agentSession.create`로 바로 가므로, 진입점을 실제로 없애는 것은 `canUseStructuredNativeChat()`의 `allowedAgents` 좁히기입니다 — 없으면 열 수 있다고 제시한 탭에서 에러 토스트가 납니다. ⚠️ **남는 것**: 이것은 egress 통제가 아니라 **도달 통제**입니다 — 허용된 에이전트의 자식 프로세스는 여전히 `enforceNetworkAllowlist` 사정권 밖입니다(#1) | 신규 RPC `src/main/runtime/rpc/methods/structured-agent-session.ts`, capability 전용 게이트 `structured-agent-session-gate.ts`, 정책 없는 생성 경로 `src/main/runtime/orca-runtime.ts`의 `resolveStructuredAgentSessionCreateIntent`/`resolveStructuredAgentSessionIntent`, 스폰 `src/main/codex/codex-structured-launch-resolution.ts:59` → `codex-app-server-connection.ts:1,67`, 도달 판정 `src/main/codex/codex-structured-location-support.ts:5`, 렌더러 전용 거부 `src/renderer/src/lib/structured-native-chat-availability.ts`, 대조군(게이트가 있는 구 레인) `orca-runtime.ts`의 `ensureAgentSession`·`createAgentSession`. **게이트**: attach funnel `structured-agent-session-attach-orchestration.ts`의 `agentBlockedByPolicyError`, 스폰 직전 `codex-structured-launch-resolution.ts`의 `assertAgentAllowedByEnterprisePolicy(record.provider)`, 가용성 프로브 `orca-runtime.ts`의 `getStructuredAgentSessionCreateSupport`, 렌더러 좁히기 `src/renderer/src/lib/structured-native-chat-availability.ts`. 동작 테스트 `src/main/runtime/structured-agent-session-enterprise-policy.test.ts`, `src/renderer/src/lib/structured-native-chat-availability.test.ts` |

| 32 | **structured 에이전트 세션 → Claude Agent SDK 자식 프로세스** (#31의 두 번째 provider. egress가 아니라 **허용목록 우회 경로**와 **메인 프로세스에 실리는 벤더 번들** 항목입니다) | ① 사용자 자신의 `claude` CLI 가 정하는 목적지 — Orca는 목적지를 모릅니다(#1과 같은 자식 프로세스 레인). ② SDK 번들 자신이 이론상 열 수 있는 벤더 엔드포인트(`api.anthropic.com` OAuth 2건, 원격 세션 서버 `${serverUrl}/sessions` 2건) | `agentSession.create`/`ensure`가 claude 를 대상으로 불릴 때, 그리고 **이미 저장된 레코드가 wake·소유자 교체·재시작 복원에서 다시 스폰될 때**. 후자는 attach 를 두 번 지나지 않습니다 | ✅ **v1.4.198에서 열렸고 같은 머지에서 닫았습니다.** attach 퍼널(`agentBlockedByPolicyError`)·가용성 프로브(`getStructuredAgentSessionCreateSupport`)·렌더러 좁히기(`agent-launch-routing.ts`)는 전부 provider-agnostic 이라 claude 도 이미 덮고 있었지만, **codex 쌍둥이에만 있던 마지막 스폰 게이트가 claude 쪽에는 없었습니다** — 그것이 재스폰 경로의 유일한 refusal 입니다. 같은 게이트를 넣고 행동 테스트 2건 + 뮤테이션 검증을 붙였습니다. **SDK 바이너리는 설치되지 않습니다** — 플랫폼별 CLI 8종이 `pnpm-workspace.yaml` 의 `ignoredOptionalDependencies` 로 제외되고, Orca 가 자식 프로세스를 직접 만들어 넘기므로 스폰 대상은 항상 사용자 자신의 CLI 입니다(`pathToClaudeCodeExecutable`). 🔴 **잔여 1**: 허용된 에이전트의 자식 프로세스는 여전히 `enforceNetworkAllowlist` 사정권 밖입니다(#1과 동일). 🔴 **잔여 2**: `sdk.mjs`(1.4MB 미니파이)가 **메인 프로세스에 동적 import** 되고 번들에 벤더 호스트 리터럴이 들어 있습니다. 네 개의 실제 호출은 Orca 가 지나지 않는 경로이고 **전부 전역 `fetch`** 라 허용목록이 볼 수 있지만(`undici`/`node:https` 사용 0건), **`new WebSocket` 1건(원격 세션 브리지)은 fetch 래퍼도 세션 가드도 보지 못합니다.** 이 판정은 미니파이 번들에 대한 정적 추론입니다 | 게이트 `src/main/claude/claude-structured-launch-resolution.ts`의 `assertAgentAllowedByEnterprisePolicy(record.provider)`, 대조군 `src/main/codex/codex-structured-launch-resolution.ts:48`, 스폰 `src/main/claude/claude-agent-sdk-process-spawn.ts`, 동적 import `src/main/claude/claude-stream-json-connection.ts:34`, 바이너리 제외 `pnpm-workspace.yaml`의 `ignoredOptionalDependencies`, 동작 테스트 `src/main/runtime/structured-agent-session-enterprise-policy.test.ts`의 `claude spawn resolver` |
...
| v1.4.188 → v1.4.190 | ✅ **0건.** `git diff --diff-filter=A --name-only`에 `updater`/`update-recovery`/`release-channel` 매치 없음, 추가 라인에도 `electron-updater`/`autoUpdater`/`checkForUpdates`/`feedUrl`/`releases/tag`/`downloadUrl`/`releaseChannel` 매치 없음. 머지 트리에 남은 두 매치는 둘 다 "이 빌드에 `electron-updater`는 의존성이 아니다"라고 적은 **주석**입니다(`src/main/app-update/app-update-check-service.ts:10`, `src/shared/enterprise-policy.ts:23`). (v1.4.185 → v1.4.188 구간은 이 표에 기록되지 않았습니다) |
...
Electron은 `spellcheck`를 기본 켜며, **Windows/Linux에서 Chromium이 hunspell 사전을 Google CDN에서 내려받습니다** — 이 문장은 코드 주석에 그대로 있습니다(`src/main/window/createMainWindow.ts:305`). 주석은 macOS를 언급하지 않습니다(macOS가 OS 검사기를 쓴다는 것은 Electron 플랫폼 동작이며 이 저장소 코드로는 확인되지 않습니다). `disableSpellcheck`는 **자체 세션을 갖는 WebContents 6곳을 전부** 끕니다 — 메인 창(`:306`), `will-attach-webview` 게스트(`:494`, 게스트는 자체 세션이라 메인 창 설정이 안 미침 — `:493` 주석), 대시보드 팝아웃 창(`src/main/window/dashboard-popout-window.ts:181`), 분리된 탭 창(`src/main/window/tab-popout-window-factory.ts:49`, 전용 파티션 `orca-tab-popout`이라 마찬가지로 상속되지 않음), 오프스크린 브라우저 백엔드(`src/main/browser/offscreen-browser-backend.ts:45`), PDF 내보내기 WebContents(`src/main/lib/html-to-pdf.ts:46`). 한 곳이라도 켜져 있으면 그 세션이 hunspell 다운로드를 다시 무장시키기 때문입니다(`html-to-pdf.ts:45` 주석).
...
`config/scripts/bootstrap-locale-catalog.mjs:71`에서 `translate.googleapis.com/translate_a/single`을 호출합니다. 이는 **번역 카탈로그를 생성하는 개발용 스크립트**이며 앱 런타임 코드가 아닙니다(`src/` 아래 어디에도 이 호스트가 없음). 사내 배포 위험 아님.
