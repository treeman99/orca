...
>
> ⚠️ **이 구간의 판정도 정적 분석입니다.** 코드 경로로 판정했고 패킷 캡처는 하지 않았습니다. **이 판에서 `file:line` 인용을 전면 재검증하지는 못했습니다** — 해체 규모상 이 문서와 README의 기존 인용 상당수가 밀려 있습니다(예: §0.2 #19의 `src/main/index.ts:2703`, `register-core-handlers.ts:201`, #20의 `browser-manager.ts:744-788`). 인용을 따라가기 전에 심볼로 다시 grep하십시오.
