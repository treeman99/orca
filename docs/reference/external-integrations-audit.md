...
>
> 🟡 **제품이 아니라 저장소·CI에 새 표면이 하나 들어왔습니다 — `docs/site/` + `.github/workflows/docs.yml`.** 업스트림이 Next.js 문서 사이트(신규 115파일, 자체 `package.json`·`pnpm-lock.yaml`)와 그것을 **Vercel `www.onorca.dev/docs`로 배포하는 워크플로**를 들여왔습니다. **데스크톱 번들에는 들어가지 않습니다**(`config/electron-builder.config.cjs:152`의 `'!docs{,/**/*}'`, `src/`에서 참조 0건). **루트 워크스페이스에도 들어가지 않습니다**(`pnpm-workspace.yaml`의 `packages: []`, 루트 lockfile에 `fumadocs|vercel@|next@16` 매치 0건 — 개발자가 루트에서 `pnpm install`을 돌려도 이 트리는 안 받습니다). **Vercel 배포 레인은 이 포크에서 도달 불가**입니다 — `production` 잡이 `needs: release_gate`이고 그 `release_gate`가 `github.repository == 'stablyai/orca'`로 막혀 있습니다(`.github/workflows/docs.yml:82, 204-208`, 업스트림이 모든 릴리스 워크플로에 쓰는 관례). 남는 도달 경로는 `docs/site/**`를 건드리는 PR에서만 도는 `check` 잡이 `docs/site/pnpm-lock.yaml` 기준 의존성 트리를 npm에서 받는 것뿐이며(저장소 게이트 없음, `:39`), **배포 산출물에는 미포함**이므로 §8과 같은 CI 전용 항목입니다. 신규 서드파티 액션은 `pnpm/action-setup@v5` 1건이고 가변 태그 핀이지만 이 저장소의 기존 관례(`actions/checkout@v6` 등)와 동일합니다.
>
...
>
> 🟡 **방어심도 관찰 1건(심각도 낮음, 미조치).** 신규 IPC `runtimeEnvironments:retryControlConnection`(`src/main/ipc/runtime-environment-connectivity-handlers.ts:142-150`)에는 `assertRemoteOrcaServerAllowed()`가 없습니다 — 같은 파일의 형제 핸들러 2곳(`:73`, `:84`)은 들고 있어 파일 안 패턴이 비대칭입니다. **현재는 무해합니다**: 본문이 `retryRemoteRuntimeSharedControlConnectionNow()` 하나뿐이고 그 구현은 `sharedControlConnections.get(id)?.connection.retryNow()`(`runtime-environment-request-connections.ts:115-117`)라 **캐시에 이미 있는 연결만** 재시도하는데, 그 캐시를 채우는 `getSharedControlConnection()`의 호출지점 3곳은 전부 `disableRemoteOrcaServer` 게이트 뒤에 있습니다(`transport-routing.ts:68`·`:144`·`:252`). 즉 정책이 켜지면 맵이 비어 있어 무조건 no-op입니다. 같은 형태의 무게이트 채널은 v1.4.194에도 있었습니다(`runtimeEnvironments:retryConnectionsNow`, 복수형). **남기는 이유**: 안전성이 자기 게이트가 아니라 "캐시는 게이트 뒤에서만 채워진다"는 원격 불변식에 의존하므로, 업스트림이 게이트 밖에서 shared-control 연결을 만드는 순간 게이트 없는 재연결 레인이 됩니다. 그 파일은 이미 가드를 import하고 있어(`:17`) 한 줄로 닫을 수 있습니다.
