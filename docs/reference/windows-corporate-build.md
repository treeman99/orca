...
| pnpm | **11.25.0** | `packageManager`가 `pnpm@11.25.0`으로 핀되어 있습니다. `npm install -g pnpm@11.25.0` 한 줄이면 됩니다 — pnpm 11은 순수 JS라 별도 네이티브 실행파일을 받지 않으므로 사내 프록시 환경에서 막히지 않습니다. **`corepack enable` 은 하지 마십시오** (shim이 이 pnpm을 가립니다). 배경과 예외 상황은 [pnpm 설치](./pnpm-12-corepack-install.md) |
...
| `error MSB8040: 이 프로젝트에는 스펙터 완화된 라이브러리가 필요합니다` | **VS에 Spectre 라이브러리를 설치하지 마세요 — 증상만 가립니다.** 원인은 pnpm 패치 미적용입니다. `node-pty`와 `@vscode/windows-process-tree`의 upstream `binding.gyp`는 `SpectreMitigation: Spectre`를 요구하고, `config/patches/`의 패치가 그 블록을 지웁니다. 패치가 적용되지 않으면 요구가 되살아나 node-gyp이 이 에러로 죽습니다. 확인: `node -e "process.exit(require('fs').readFileSync('node_modules/@vscode/windows-process-tree/binding.gyp','utf8').includes('SpectreMitigation')?1:0)"` — exit 1이면 미적용입니다(`node_modules/node-pty/binding.gyp`도 같은 방법으로 확인). 조치는 `pnpm install` 재실행이고, 그래도 남으면 패치 파일 자체가 깨진 것이므로 `pnpm test config/scripts/pnpm-patch-integrity.test.mjs`로 판정하십시오. **pnpm은 깨진 hunk를 조용히 건너뛰고 exit 0으로 끝냅니다** — `ignorePatchFailures: false`로도 잡히지 않아 저장소 게이트가 대신 잡습니다 |
| `error C1083: 포함 파일을 열 수 없습니다 'napi.h'` (`addon.cc` 등) | 위 패치가 `include_dirs`를 `deps/node-addon-api`로 돌려놓기 때문에, 그 디렉터리에 헤더를 넣어 주지 않으면 컴파일이 여기서 죽습니다. node-addon-api를 직접 참조할 수는 없습니다 — `require('node-addon-api').include_dir`과 `.targets` 모두 cwd 기준 상대 경로(`../../../../node-addon-api@7.1.0/...`)를 돌려주고 node-gyp이 그걸 pnpm 스토어 realpath에서 해석합니다. `config/scripts/ensure-windows-process-tree-source.mjs`가 헤더를 스테이징하며, `rebuild-native-deps.mjs`(postinstall·`beforeBuild`)와 릴레이 애드온 빌드 양쪽이 이를 호출합니다. `pnpm install` 재실행으로 해결되고, 그래도 나면 `pnpm test config/scripts/ensure-windows-process-tree-source.test.mjs`로 판정하십시오 |
...
| `EPERM ... .node` | 실행 중인 Orca/Electron 프로세스가 파일을 잡고 있음. 전부 종료 후 재시도. 백신 실시간 검사가 방금 컴파일한 `.node`를 잠깐 잡아 같은 오류를 내는 경우도 있습니다(사내 재현은 안 됐고 **추정**입니다) — 반복되면 저장소와 `%TEMP%`를 검사 예외에 넣으십시오 |
| `EPERM ... rename` (스택에 `binding.rename`) | **사내에서 실제로 관측된 실패이며, 지점은 `config/scripts/project-renderer-web-client.mjs`의 web 산출물 교체입니다.** 원인은 드라이브 경계가 **아닙니다** — staging(`out/.web-projection-<pid>`)과 `out/web`은 언제나 같은 볼륨입니다. 원인은 핸들 잠금입니다: Windows [`RemoveDirectory`](https://learn.microsoft.com/windows/win32/api/fileapi/nf-fileapi-removedirectoryw)는 "닫힐 때 삭제"로 표시만 하므로, 백신·Search Indexer·탐색기가 핸들을 쥐고 있으면 `out/web` 삭제가 성공을 반환한 뒤에도 이름이 남고, 뒤이은 디렉터리 rename이 거부됩니다([`MOVEFILE_REPLACE_EXISTING`은 대상이 디렉터리면 에러](https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-movefileexw#parameters)). 이 브랜치는 rename을 없애고 파일 단위 copy + JS 재시도(`withFsRetry`)로 바꿨습니다. **`$env:TEMP` 우회는 이 증상에 효과가 없습니다.** 반복되면 저장소와 `out\`을 백신 실시간 검사 예외에 등록하십시오 |
| `EPERM`/`EXDEV`가 `node_modules\electron` 설치 중 (`install-electron-package-binary.mjs`) | 위와 **다른 실패**이며 사내 관측 사례는 없습니다(**예방 조치**). `%TEMP%`와 저장소가 다른 드라이브면 Electron dist 트리를 디렉터리째 옮길 수 없습니다 — Windows `MoveFileEx` 문서상 "디렉터리는 대상이 같은 드라이브여야" 합니다. 추출 스테이징을 `node_modules/electron/.orca-extract-<pid>`로 옮겨 항상 같은 볼륨에서 끝나게 했고, copy 폴백도 남겨 두었습니다. 그래도 나면 임시 폴더를 저장소와 같은 드라이브로 지정하십시오 — `$env:TEMP = 'C:\src\tmp'; $env:TMP = $env:TEMP` (`os.tmpdir()`는 `TEMP` → `TMP` 순으로 읽습니다) |
