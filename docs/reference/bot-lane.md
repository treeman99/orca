...
- 이 포크는 `experimentalNativeChat`을 **기본 on**으로 내보냅니다. 꺼져 있으면 봇 챗이 조용히
  ...
  **기본 off로 유지**해야 합니다 — 그게 켜지면 봇이 아닌 일반 세션까지 채팅으로 열립니다.
  봇만 채팅인 이유는 이 레인이 자기 페인의 `viewMode`를 직접 세팅하기 때문입니다.
...
6. **채팅 뷰**: 봇을 열면 곧바로 말풍선이어야 합니다(`Chat UI`는 기본 on). 같은 워크스페이스의
   일반 세션을 더블클릭하면 **터미널**이어야 합니다 — 여기가 채팅으로 열리면
   설정 → Experimental → `Chat UI` → `Default view`가 `Chat UI`로 남아 있는 프로필입니다.
