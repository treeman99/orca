...
```
봇 = 이름 + 아바타(고르지 않으면 이름에서 생성한 얼굴) + 역할 설명
    + 에이전트 선택 (TuiAgent)
...
- 배경 세션 탭은 `launchAgent`를 **탭에도** 기록합니다(`adopt-agent-background-session-tab.ts`).
  ...
  실기에서 확인한 증상: 더블클릭 직후 잠깐 말풍선이 보였다가 TUI로 바뀜.
- 이 포크는 `experimentalNativeChat`을 **기본 on**으로 내보냅니다. 꺼져 있으면 봇 챗이 조용히
  터미널로 되돌아가서 "채팅 UI가 안 나온다"로만 보입니다. 짝이 되는 `openAgentTabsInChatByDefault`는
  **기본 off**입니다 — 켜지면 봇이 아닌 일반 세션까지 채팅으로 열립니다.
- **탭바 `+` 메뉴는 그 설정을 아예 보지 않습니다.** 에이전트 행은 `forceTerminalView`로 항상
  터미널을, 그 아래 `봇 대화` 섹션은 그 프로젝트 봇을 골라 채팅을 엽니다. 이유: 프로필에 남은
  `openAgentTabsInChatByDefault: true`(설치 사이에도 따라옵니다) 하나 때문에 `+ → Claude`가
  채팅으로 열리는데, 메뉴만 봐서는 왜 그런지 알 방법이 없었습니다. 한 메뉴에서 두 선택지를
  나란히 두면 숨은 모드가 사라집니다. 봇 행은 `openBotConversation`을 호출하므로 로스터
  더블클릭과 완전히 같은 경로입니다.
- 세션 생성(워크스페이스 만들기·초안 실행 등) 경로는 여전히 그 설정을 존중합니다. 거기까지
  끄면 draft 미러링·워크트리 시드 불변식 테스트 45건이 함께 무너집니다 — 그 기계장치가
  다른 것도 떠받치고 있습니다.
...
6. **채팅 뷰**: 봇을 열면 곧바로 말풍선이어야 합니다(`Chat UI`는 기본 on).
   탭바 `+` → `Claude`는 **터미널**, `+` → `봇 대화` → 봇 이름은 **채팅**이어야 합니다.
   설정 → Experimental → `Chat UI` → `Default view`를 `Chat UI`로 바꿔도 `+`의 에이전트 행은
   터미널이어야 합니다(그게 이 분리의 요점입니다).
   설정 → Experimental → `Chat UI`를 끄면 봇도 터미널로 돌아옵니다.
   지원 목록 밖 에이전트(예: gemini) 봇은 켜도 터미널입니다.
