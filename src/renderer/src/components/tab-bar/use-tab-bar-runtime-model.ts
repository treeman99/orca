...
export function useTabBarRuntimeModel({
  ...
  // Why the predicate: the raw setting read leaves the corporate `disableMobileEmulator`
  // policy out, and this hook feeds every create-menu affordance in the tab bar.
  ...
  const agentLaunchOptions = useMemo(
    () =>
      buildTabAgentLaunchOptions(
        orderTabLaunchAgents(defaultAgent, detectedIds ?? [], disabledTuiAgents, allowedAgents),
        agentCmdOverrides
      ),
    [agentCmdOverrides, allowedAgents, defaultAgent, detectedIds, disabledTuiAgents]
  )
