...
export function bindFreshSpawnFollowReset(session: ConnectPanePtySession): void {
  ...
  session.hasRestoredViewportBlankingMarker = (): boolean => {
    return session.deps.restoredViewportBlankingPanesRef?.current.has(session.pane.id) ?? false
  }
  ...
  session.writeRestoredViewportReset = (args: {
    ownerProcessEnded: boolean
    rows?: number
  }): void => {
    session.writeReplayData(
      buildRestoredViewportResetSequence({
        rows: args.rows ?? session.pane.terminal.rows,
