...
describe('settings navigation metadata', () => {
  ...
  it('places Automations first under Workflows and omits both removed vendor sharing panes', () => {
    const sections = buildSettingsNavigationMetadata({
      isMac: false,
      isWindows: false,
      isWebClient: false,
    ...
    // Dropping them here is what also keeps them out of settings search and the Cmd+J palette.
    expect(sections.find((section) => section.id === 'artifacts')).toBeUndefined()
    expect(sections.find((section) => section.id === 'share-skills')).toBeUndefined()
    expect(workflowIds[0]).toBe('automations')
    expect(
      sections.flatMap((section) => section.searchEntries).map((entry) => entry.title)
    ).not.toContain('Unlisted skill links')
  })
