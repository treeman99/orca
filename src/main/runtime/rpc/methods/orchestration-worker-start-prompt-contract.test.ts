...
describe('orchestration worker-start prompt contract', () => {
  ...
  // Removed for this fork: upstream reports a stalled submit by throwing immediately, while this
  // fork first lets the rescue resend one Enter (assertAgentPromptRescuedIfStalled — only an
  // `unverified` submit still raises `agent_prompt_stalled`). The fork's behaviour is the superset,
  // so this case contradicts it by construction. Expect it back on the next upstream sync.
})
