...
describe('a worker cannot tell which mode it is running in', () => {
  ...
  it('lets a structured worker dispatch a sub-worker like any other coordinator', async () => {
    ...
    // By handle, not call count: this fork reads the *worker's* own title for a readiness
    // diagnostic. The invariant is that the coordinator's `--from` never needs a PTY.
    expect(showTerminal).not.toHaveBeenCalledWith('structworker_coord')
