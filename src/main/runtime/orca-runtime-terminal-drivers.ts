...
import { assertMobileEmulatorAllowedByPolicy } from '../enterprise/mobile-emulator-guard'
...
export class OrcaRuntimeWithTerminalDrivers extends OrcaRuntimeWithFitOverrideListeners {
  ...
  protected readonly edgeCommands = new RuntimeEdgeCommandController({
    ...
    emulatorHost: {
      // Policy gate on the one funnel every emulator RPC handler pulls the bridge through
      // (requireEmulatorBridge) — covers the `orca emulator` CLI and headless `orca serve`.
      getEmulatorBridge: () => {
        assertMobileEmulatorAllowedByPolicy()
