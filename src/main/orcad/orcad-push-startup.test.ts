...
// Fork: the real resolver reports no vendor gateway (push-gateway-removal.test.ts); this suite pins
// the headless push lifecycle, so it supplies an origin of its own.
vi.mock('../runtime/push/push-gateway-origin', () => ({
  resolvePushGatewayOrigin: () => 'https://push.gateway.test'
}))
