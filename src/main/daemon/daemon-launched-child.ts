...
import type { DaemonLaunchHost } from './daemon-launch-hosts'
import { logDaemonLaunch } from './daemon-launch-log'
...
type LaunchDaemonChildOptions = {
  ...
  /** Which image this attempt forks from; log label only. */
  hostKind: DaemonLaunchHost['kind']
  ...
  const startupSignal = new Promise<void>((resolve, reject) => {
    ...
    function onReadyMessage(msg: unknown): void {
      ...
      if (msg && typeof msg === 'object' && (msg as { type?: string }).type === 'ready') {
        ...
        settled = true
        logDaemonLaunch('launch-ready', { host: hostKind })
