...
import { installManagedAgentHooksUnderEnterprisePolicy } from './agent-hooks/enterprise-agent-hook-policy'
...
import { setMainTranslator } from './i18n/main-translation-port'
import {
...
function getExpectedTeardownScope(
  ...
  return resolveExpectedTeardownScope({
    isQuitting,
    // The enterprise build strips ./updater, so there is never an update quit.
...
void app.whenReady().then(async () => {
  ...
  if (shouldReconcileStartupManagedHooks) {
    ...
    void realHomeCodexHookState
      .then(() =>
        installManagedAgentHooksUnderEnterprisePolicy(managedHookStore.getSettings(), {
  ...
  // Why through a port: modules the Orca runtime reaches may not import main-i18n, which
  // pulls electron in (see src/main/i18n/main-translation-port.ts).
  setMainTranslator(translateMain)
