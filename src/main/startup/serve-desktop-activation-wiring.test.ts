...
describe('serve desktop activation wiring', () => {
  ...
  it('wires no update-install policy into desktop promotion', () => {
    expect(windowCoreSource).not.toContain('updateInstallMode')
    expect(windowCoreSource).not.toContain('resolveUpdateInstallMode')
    expect(windowCoreSource).not.toContain('ensureAutoUpdaterConfigured')
  })
})
