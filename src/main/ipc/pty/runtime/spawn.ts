...
export async function spawnPtyFromRuntimeController(
  ...
  // The non-renderer twin of the `pty:spawn` gate: the CLI, a paired mobile client, an
  // automation run and an orchestration dispatch all reach a PTY through this controller
  // instead, so the allowlist has to be checked on both entry points.
  if (args.launchAgent) {
