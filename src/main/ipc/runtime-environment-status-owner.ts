...
import { remoteOrcaServerRefusal } from '../enterprise/remote-orca-server-guard'
...
export function createRuntimeEnvironmentStatusOwner(
  ...
  return new RuntimeHostStatusOwner({
    ...
    request: (signal) => {
      // Fork: every status probe to a saved remote Orca passes here — including the boot loop in
      // runtime-environments.ts, which bypasses transport-routing's three guarded entry points.
