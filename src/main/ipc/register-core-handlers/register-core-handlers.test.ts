...
const getEnterprisePolicyMock = vi.hoisted(() =>
  vi.fn(() => ({ disablePlugins: false, disableMobileEmulator: false, disableVoice: false }))
)
...
// Fork: these three registrars are gated on the corporate policy at the registration site,
// because each one bypasses the RPC dispatcher and reaches the network or a device directly.
...
describe('registerCoreHandlers', () => {
  beforeEach(() => {
    ...
    registerPluginHandlersMock.mockReset()
  ...
  it('passes the store through to handler registrars that need it', async () => {
    ...
    // Fork: vendor feedback and Bitbucket store credentials and egress — the modules are gone.
    expect(registerBitbucketHandlersMock).not.toHaveBeenCalled()
    ...
    expect(registerFeedbackHandlersMock).not.toHaveBeenCalled()
    ...
    // Why runtime: a tab close has to end that tab's PTYs and worker dispatches, which only
    // the runtime can reach — without it the close de-persists a surface whose sessions live on.
    ...
    // Fork: the updater lane is removed from the build.
    expect(registerUpdaterHandlersMock).not.toHaveBeenCalled()
  ...
  it('does not register the emulator, speech, or plugin handlers under a lockdown policy', async () => {
    // Why a behavioural test and not a resolver one: this loss already happened once. Upstream
    // moved register-core-handlers.ts into a directory and the fork's three `if (!policy...)`
    // guards did not come with it — no conflict, no type error, green suite.
    getEnterprisePolicyMock.mockReturnValue({
      disablePlugins: true,
    ...
    const runtime = { marker: 'runtime', getAgentBrowserBridge: () => null }
    const pluginService = { marker: 'pluginService', reconcileActivationState: vi.fn() }
    ...
    registerCoreHandlersFresh(
      ...
      undefined,
      undefined,
      { marker: 'agentAwakeService' } as never,
      pluginService as never,
      undefined,
      { getAdditionalAiVaultCodexHomePaths: vi.fn(() => []), onBeforeRelaunch: vi.fn() }
    )

    expect(registerEmulatorFrameStreamHandlersMock).not.toHaveBeenCalled()
    expect(registerEmulatorVideoStreamHandlersMock).not.toHaveBeenCalled()
