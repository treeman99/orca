...
export async function listReleaseBuilds(channel: ReleaseChannel): Promise<ReleaseBuild[]> {
  // Why here and not at the updater:listBuilds handler: this is the only net.fetch in the
  // build-picker lane, so a future caller cannot reach api.github.com around the lockdown.
  if (getEnterprisePolicy().disableAutoUpdate) {
    throw new Error('Build switching is turned off by your organization’s Orca policy.')
  }
