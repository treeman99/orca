...
/** Fork: extract staging lives beside dist, so a killed build leaves these behind. */
...
describe('install-electron-package-binary', () => {
  // Fork guard: %TEMP% is frequently on another Windows volume, and a directory
  // cannot be renamed across volumes — so the tree we rename into dist is staged
  // beside dist, never in the download temp directory.
