...
/** One `## <title>` section of the kernel, up to the next heading. Fork-owned assertions use it. */
...
describe('orchestration kernel', () => {
  it('keeps the always-loaded guide compact and ordered around the normal protocol', () => {
    ...
    const headings = [
      ...
      // Fork-owned, and deliberately in the kernel: a rule the coordinator only reads
      // conditionally is a rule nothing ever injects.
      '## Project Rule Ledger',
      '## Conditional references'
    ]

    // Why: 202 upstream + 15 for this fork's rule ledger; the kernel is always in context.
    expect(kernel.split(`\n`).length).toBeLessThanOrEqual(218)
